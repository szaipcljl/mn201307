 ...
 */
#ifndef _ROADCAMERA3OEMIF_H
#define _ROADCAMERA3OEMIF_H
...
//#include "RoadCameraParameters.h"  //TODO:
#include "RoadOEMCamera.h" //TODO
//#include "cmr_oem.h"
//#include "road_dma_copy_k.h"

#include "RoadCamera3Setting.h"
#include "RoadCamera3Stream.h"
#include "RoadCamera3Channel.h"
...
typedef struct road_camera_memory {
	...
	void *data;
	bool busy_flag;
} road_camera_memory_t;

#ifdef CONFIG_ROAD_PRIVATE_ZSL
struct ZslBufferQueue {
	camera_frame_type frame;
	road_camera_memory_t* heap_array;
	uint32_t valid;
};
#endif
...
typedef struct road_zsl_capture_info {
	struct frm_info zslCapInfo;
	cmr_uint addr_vir;
}road_zsl_capture_info_t;
...
class RoadCamera3OEMIf : public virtual RefBase {
public:
	RoadCamera3OEMIf(int cameraId, RoadCamera3Setting *setting);
	virtual ~RoadCamera3OEMIf();
	...
	road_camera_memory_t *allocCameraMem(int buf_size, int num_bufs, uint32_t is_cache);
...
private:
	...
	void freeCameraMem(road_camera_memory_t *camera_memory);
	...
	enum Road_camera_state {
		ROAD_INIT,
		ROAD_IDLE,
		ROAD_ERROR,
		ROAD_PREVIEW_IN_PROGRESS,
		ROAD_FOCUS_IN_PROGRESS,
		ROAD_SET_PARAMS_IN_PROGRESS,
		ROAD_WAITING_RAW,
		ROAD_WAITING_JPEG,
		ROAD_FLASH_IN_PROGRESS,

		// internal states
		ROAD_INTERNAL_PREVIEW_STOPPING,
		ROAD_INTERNAL_CAPTURE_STOPPING,
		ROAD_INTERNAL_PREVIEW_REQUESTED,
		ROAD_INTERNAL_RAW_REQUESTED,
		ROAD_INTERNAL_STOPPING,
	...
	typedef struct _camera_state {
		Road_camera_state      camera_state;
		Road_camera_state      preview_state;
		Road_camera_state      capture_state;
		Road_camera_state      focus_state;
		Road_camera_state      setParam_state;
	} camera_state;
	...
	const char *getCameraStateStr(Road_camera_state s);
	Road_camera_state transitionState(Road_camera_state from,
						Road_camera_state to,
						state_owner owner,
						bool lock = true);
	void setCameraState(Road_camera_state state, state_owner owner = STATE_CAMERA);
	inline Road_camera_state getCameraState();
	inline Road_camera_state getPreviewState();
	inline Road_camera_state getCaptureState();
	inline Road_camera_state getFocusState();
	inline Road_camera_state getSetParamsState();
	...
	int overwritePreviewFrameMemInit(struct RoadCamera3OEMIf::OneFrameMem *one_frame_mem_ptr);
	...
	int allocOneFrameMem(struct RoadCamera3OEMIf::OneFrameMem *one_frame_mem_ptr);
	int relaseOneFrameMem(struct RoadCamera3OEMIf::OneFrameMem *one_frame_mem_ptr);
	...
	int map(road_camera_memory_t* camera_memory, hal_mem_info_t *mem_info);
	int unmap(road_camera_memory_t* camera_memory, hal_mem_info_t *mem_info);
	...
	bool                              mRoadZslEnabled;
	bool                              mVideoSnapshotRoadZslEnabled;
	...
	*/
	...
	static const int                kISPB4awbCount         = 2;
	...
	road_camera_memory_t            *mRawHeap;
	uint32_t                        mRawHeapSize;

	road_camera_memory_t            *mSubRawHeapArray[MAX_SUB_RAWHEAP_NUM];
	road_zsl_capture_info_t                 mZSLCapInfo[MAX_SUB_RAWHEAP_NUM];
	road_camera_memory_t            *mReDisplayHeap;
	//TODO: put the picture dimensions in the CameraParameters object;
	//RoadCameraParameters            mParameters;
	...
	*/
	...
	road_camera_memory_t*           mPreviewHeapArray[kPreviewBufferCount+kPreviewRotBufferCount+1];
	road_camera_memory_t*           mVideoHeapArray[kVideoBufferCount+kVideoRotBufferCount+1];
	road_camera_memory_t*           mZslHeapArray[kZslBufferCount+kZslRotBufferCount+1];
#ifdef CONFIG_ROAD_PRIVATE_ZSL
	...
	road_camera_memory_t*           mCommonHeapReserved;
#else
	road_camera_memory_t*           mPreviewHeapReserved;
	road_camera_memory_t*           mVideoHeapReserved;
	road_camera_memory_t*           mZslHeapReserved;
#endif
	road_camera_memory_t*           mIspLscHeapReserved;
	road_camera_memory_t*           mIspB4awbHeapReserved[kISPB4awbCount];

	uint32_t                        mPreviewHeapBakUseFlag;
	road_camera_memory_t            mRawHeapInfoBak;
	...
	uint32_t			mShinWhitenNotDetectFDNum;
#endif
};

}; // namespace roadcamera

#endif //_ROADCAMERA3_OEMIF_H
