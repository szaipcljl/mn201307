...
#define PREV_TIMEOUT         3000000000     /*3000ms*/
...
bool SprdCamera3OEMIf::WaitForBurstCaptureDone()
{
	...
	while (NO_ERROR == jpeg_ret || NO_ERROR == callback_ret) {
		...
		if((mTakePictureMode == SNAPSHOT_PREVIEW_MODE || mTakePictureMode == SNAPSHOT_ONLY_MODE)
			&& mCameraState.capture_state == SPRD_IDLE){
			return true;
		}
		if (mStateWait.waitRelative(mStateLock, CAP_TIMEOUT)) {
			HAL_LOGE("WaitForBurstCaptureDone timeout");
			return false;
		}
	...
	if(channel) {
		...
		if(mSprdZslEnabled) {
			...
			if (PREVIEW_ZSL_FRAME == frame->type) {
				...
				if (!isCapturing() || !need_pause || mRecordingMode == true) {
