...
static int setup_virtual_keys(input_device *dev)
{
    ...
    for(i=0;i<text_tp_rows;i++){
        if(0 == i)
            sprintf(item_key,"0x01:%d:%d:%d:%d:%d",KEY_VIR_ITEMS+i,width/2,(i+1)*CHAR_HEIGHT+(CHAR_HEIGHT>>1),width,CHAR_HEIGHT);
        else sprintf(item_key,"%s:0x01:%d:%d:%d:%d:%d",item_key,KEY_VIR_ITEMS+i,width/2,(i+1)*CHAR_HEIGHT+(CHAR_HEIGHT>>1),width,CHAR_HEIGHT);
    }
    sprintf(vir_key,"0x01:%d:%d:%d:%d:%d:0x01:%d:%d:%d:%d:%d:0x01:%d:%d:%d:%d:%d:%s",KEY_VIR_PASS,height>>4,height-(height>>4),height>>3,height>>3,
		KEY_VIR_FAIL,width-(height>>4),height-(height>>4),height>>3,height>>3,KEY_VIR_NEXT_PAGE,width/2,height-(height>>4),height>>3,height>>3,item_key);
    #endif
    f = fopen(vkpath, "r");
    if (f != NULL) {
...
#if DEBUG_TOUCH_EVENTS
                SPRD_DBG("[Touch] Virtual key event: code=%d, touch-x=%d, touch-y=%d",
