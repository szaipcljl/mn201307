...
extern SENSOR_INFO_T g_gc2365_mipi_raw_info; //caolina  160510 add 
extern SENSOR_INFO_T g_ov8856_mipi_raw_info;      //caolina  160510 add 
...
const SENSOR_MATCH_T sub_sensor_infor_tab[]=
{
	...
	{"ov5648_mipi_raw", &g_ov5648_mipi_raw_info},
	{"ov2680_mipi_raw", &g_ov2680_mipi_raw_info},
	{"gc2365_mipi_raw", &g_gc2365_mipi_raw_info}, 
#endif
...
#if !(defined(CONFIG_CAMERA_ISP_VERSION_V3) || defined(CONFIG_CAMERA_ISP_VERSION_V4))
#ifdef CONFIG_BACK_CAMERA_MIPI
	{"autotest_ov8825_mipi_raw", &g_autotest_ov8825_mipi_raw_info},
	{"autotest_ov5640_mipi_yuv", &g_autotest_ov5640_mipi_yuv_info},
	{"ov5648_mipi_raw", &g_ov5648_mipi_raw_info},
#endif
#ifdef CONFIG_BACK_CAMERA_CCIR
...
#else
#ifdef CONFIG_BACK_CAMERA_MIPI
        {"autotest_ov8858_mipi_raw", &g_ov8858_mipi_raw_info},
		{"ov8856_mipi_raw", &g_ov8856_mipi_raw_info},
#endif
#endif
PNULL
...
#else
#ifdef CONFIG_FRONT_CAMERA_MIPI
    {"autotest_ov2680_mipi_raw", &g_ov2680_mipi_raw_info},
#endif
#endif
PNULL
...
SENSOR_MATCH_T * Sensor_GetInforTab(struct sensor_drv_context *sensor_cxt, SENSOR_ID_E sensor_id)
{
	...
	if (AUTO_TEST_CAMERA == at_flag) {
		switch ( sensor_id) {
			...
			case SENSOR_DEV_2:
					sensor_infor_tab_ptr=(SENSOR_MATCH_T*)&at_dev2_sensor_infor_tab;
					break;
			case SENSOR_ATV:
	...
	switch (sensor_id) {
		...
		case SENSOR_SUB:
			...
			break;
		case SENSOR_DEV_2:
			sensor_infor_tab_ptr=(SENSOR_MATCH_T*)&dev2_sensor_infor_tab;
			break;
	...
	if (AUTO_TEST_CAMERA == at_flag) {
		switch (sensor_id) {
			...
			case SENSOR_SUB:
				...
				break;
			case SENSOR_DEV_2:
				tab_lenght=(sizeof(at_dev2_sensor_infor_tab)/sizeof(SENSOR_MATCH_T));
				break;
		...
		switch (sensor_id) {
			...
			case SENSOR_SUB:
				...
				break;
            case SENSOR_DEV_2:
				tab_lenght=(sizeof(dev2_sensor_infor_tab)/sizeof(SENSOR_MATCH_T));
				break;
			case SENSOR_ATV:
...
cmr_u32 Sensor_IndexGet(struct sensor_drv_context *sensor_cxt, cmr_u32 index) {
	...
	if (AUTO_TEST_CAMERA == at_flag) {
		...
		if(index == SENSOR_DEV_2) {
			sSnNum = sizeof(dev2_sensor_infor_tab)/sizeof(SENSOR_MATCH_T)-1;
			CMR_LOGI("sensor sensorTypeMatch dev2 is %d", sSnNum );
			for(i=0; i<sSnNum; i++) {
				if(strcmp(dev2_sensor_infor_tab[i].sn_name, CAMERA_SENSOR_TYPE_DEV_2) == 0) {
					CMR_LOGI("sensor dev2 matched the %dth  is %s", i, CAMERA_SENSOR_TYPE_DEV_2);
					retValue = i;
					break;
				}
			}
