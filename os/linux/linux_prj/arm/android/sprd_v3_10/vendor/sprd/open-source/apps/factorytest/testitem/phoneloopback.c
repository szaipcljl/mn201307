...
int test_assisloopback_start(void)
{
	...
	#if defined (SP7731) || defined (SP9838) || defined (SP9830AEC) || defined (SP9830IEC) || defined(SP9832IEA)
	ui_clear_rows(row, 2);
	ui_set_color(CL_WHITE);
	...
	at_cmd_audio_loop(0,0,0,0,0,0);
	usleep(5*1000);
	at_cmd_audio_loop(1,3,8,1,3,0);
	at_cmd_change_outindevice(2,2);
	#else
	ui_clear_rows(row, 2);
	ui_set_color(CL_RED);
	ui_show_text(row,0,TEST_ASSIS_NOTSUPPORT);
	gr_flip();
	sleep(1);
	return RL_NA;
	#endif
	ret = ui_handle_button(TEXT_PASS,NULL,TEXT_FAIL);//, TEXT_GOBACK
	at_cmd_audio_loop(0,3,8,1,3,0);
