...
int _emmc_real_erase_partition(uchar * partition_name)
{
	...
	if (count < EMMC_ERASE_ALIGN_LENGTH) {
		uchar buf[EMMC_ERASE_ALIGN_LENGTH * EFI_SECTOR_SIZE] = { 0xFF };
		debugf("Use Emmc_Write to erase partition because of align\n");
		if (!Emmc_Write(curArea, base_sector, count, buf))
			return 0;
	} else {
		if (base_sector % EMMC_ERASE_ALIGN_LENGTH) {
			uchar buf[EMMC_ERASE_ALIGN_LENGTH * EFI_SECTOR_SIZE] = { 0xFF };
			uint32_t base_sector_offset = 0;

			base_sector_offset = EMMC_ERASE_ALIGN_LENGTH - base_sector % EMMC_ERASE_ALIGN_LENGTH;
			debugf("Use Emmc_Write to erase partition because of base location\n");
		...
		if (0 == count)
			return 1;
		debugf("Use Emmc_Erase to erase partition\n");
...
static int _emmc_secure_download(uchar * partition_name)
{
	...
	case NORMALE_IMAGE:	// 1 boot image,uboot image, etc...
		if (strcmp("uboot", partition_name) == 0) {
			secure_verify("splloader", g_eMMCBuf, 0);
		} else {
			secure_verify("fdl2", g_eMMCBuf, 0);
		}
		break;
	case SPL_IMAGE:	//  2 spl image
		secure_verify("splloader0", g_eMMCBuf, 0);
		break;
