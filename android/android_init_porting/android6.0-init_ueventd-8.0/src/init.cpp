...
static char console_name[PROP_VALUE_MAX] = "/dev/ttyLF0";
...
void service_start(struct service *svc, const char *dynamic_args)
{
    ...
    if (pid == 0) {
        ...
        if (!dynamic_args) {
			INFO("~~~exec !dynamic_args\n");
			for(int i=0;i<svc->nargs;i++){
               INFO(" %s",svc->args[i]);
			} 
			INFO("\n");
			...
			INFO("~~~exec else!dynamic_args(%s)(%s)(%s)(%s)\n",svc->args[0],svc->args[1],svc->args[2],svc->args[3]);
        }
        _exit(127);
    }
...
void execute_one_command() {
	...
	INFO("!!!!!exec");
	for (int n = 0; n < cur_command->nargs; n++) {
	INFO(" '%s'", cur_command->args[n]);
	}
	INFO("\n");
...
{
printf("~~~~console init action function caller\n");
#if 0
   	pid_t pid;
	pid = fork();
...
static int console1_init_action(int nargs, char **args){
	#if 0
	// "/sbin/getty -L 115200 ttyLF0"
	char* const path = (char* const)"/sbin/getty";
	char* const args1[] = { path, (char* const)"-L", (char* const)"115200", (char* const)"ttyLF0", nullptr };
	//std::cout << "execv(\"" << path << "\")! "<< std::endl;
    INFO("$$$execv %s /sbin/getty\n",path);
	execv(path, args1);
	INFO("$$$execv %s /sbin/getty\n",path);
...
int main(int argc, char** argv) {
 ...
 //}
  ...
  queue_builtin_action(console1_init_action, "console1_init");
