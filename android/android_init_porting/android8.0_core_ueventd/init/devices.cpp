...
static bool match_subsystem(perms_* dp, const char* pattern,
	...
	std::string subsys_path = android::base::StringPrintf(pattern, subsystem, basename((char *)path));
...
static void make_link_init(const char* oldpath, const char* newpath) {
  ...
  if (mkdir_recursive(dirname((char*)newpath), 0755)) {
...
static void handle_generic_device_event(struct uevent *uevent)
{
    ...
    if (subsystem) {
        const char *devname;
		base = ""; //add for error: [-Werror=maybe-uninitialized]
