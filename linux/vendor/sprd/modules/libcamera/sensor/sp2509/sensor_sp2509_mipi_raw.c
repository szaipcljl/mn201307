...
#define SNAPSHOT_LINE_TIME   395 
#define PREVIEW_LINE_TIME	395 // 947(*2)/36M(*2)
...
{0xfd,0x01},
{0x9d,0xea},//
{0xa0,0x05}, //;updated by lixl on 20151231;P1:0xa0=0x29(?-?a05) ?¨´?Y2a¨º??¨¢1?¦Ì¡Â?¨²data and clock skew¡ê??¨¦??D??¨¹??
{0xa1,0x02},
{0xad,0x62},
{0xae,0x00},
 ...
 *
 *============================================================================*/
static uint32_t sp2509_set_video_mode(uint32_t param)
{
...
static uint32_t sp2509_InitRawTuneInfo(void)
{
	uint32_t rtn=0x00;
 ...
 *
 *============================================================================*/
static uint32_t sp2509_get_default_frame_length(uint32_t mode)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint16_t sp2509_read_gain(void)
{
	uint16_t gain_h = 0;
	uint16_t gain_l = 0;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp2509_write_gain(uint32_t gain)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint16_t sp2509_read_frame_length(void)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp2509_write_frame_length(uint32_t frame_len)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint32_t sp2509_read_shutter(void)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void sp2509_write_shutter(uint32_t shutter)
{
	Sensor_WriteReg(0xfd, 0x01);
 ...
 *============================================================================*/
static uint16_t sp2509_update_exposure(uint32_t shutter,uint32_t dummy_line)
{
	...
	cur_fr_len = sp2509_read_frame_length();

	if (shutter < SENSOR_MIN_SHUTTER)
		shutter = SENSOR_MIN_SHUTTER;

	if (dest_fr_len != cur_fr_len)
		sp2509_write_frame_length(dest_fr_len);
write_sensor_shutter:
	/* write shutter to sensor registers */
	sp2509_write_shutter(shutter);
	return shutter;
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint32_t sp2509_power_on(uint32_t power_on)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint32_t sp2509_identify(uint32_t param)
{
 ...
 *
 *============================================================================*/
static unsigned long sp2509_get_resolution_trim_tab(uint32_t param)
{
 ...
 *============================================================================*/
static uint32_t sp2509_before_snapshot(uint32_t param)
{
	...
	s_current_default_frame_length = sp2509_get_default_frame_length(capture_mode);
	...
	cap_shutter = sp2509_update_exposure(cap_shutter,0);
	cap_gain = gain;
	sp2509_write_gain(cap_gain);
 ...
 *============================================================================*/
static uint32_t sp2509_write_exposure(uint32_t param)
{
	...
	s_current_default_frame_length = sp2509_get_default_frame_length(mode);

	s_sensor_ev_info.preview_shutter = sp2509_update_exposure(exposure_line,dummy_line);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static uint32_t sp2509_write_gain_value(uint32_t param)
{
	uint32_t ret_value = SENSOR_SUCCESS;
	uint32_t real_gain = 0;
 ...
 *
 *============================================================================*/
static void sp2509_increase_hdr_exposure(uint8_t ev_multiplier)
{
	...
	if (shutter_multiply >= ev_multiplier) {
		sp2509_update_exposure(s_hdr_info.capture_shutter * ev_multiplier,0);
		sp2509_write_gain(s_hdr_info.capture_gain);
	} else {
		gain = s_hdr_info.capture_gain * ev_multiplier / shutter_multiply;
		sp2509_update_exposure(s_hdr_info.capture_shutter * shutter_multiply,0);
		sp2509_write_gain(gain);
	}
}
 ...
 *
 *============================================================================*/
static void sp2509_decrease_hdr_exposure(uint8_t ev_divisor)
{
	...
	if (gain_multiply >= ev_divisor) {
		sp2509_update_exposure(s_hdr_info.capture_shutter,0);
		sp2509_write_gain(s_hdr_info.capture_gain / ev_divisor);

	} else {
		shutter = s_hdr_info.capture_shutter * gain_multiply / ev_divisor;
		sp2509_update_exposure(shutter,0);
		sp2509_write_gain(s_hdr_info.capture_gain / gain_multiply);
	}
}
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static uint32_t sp2509_set_hdr_ev(unsigned long param)
{
	...
	case SENSOR_HDR_EV_LEVE_0:
		ev_divisor = 2;
		sp2509_decrease_hdr_exposure(ev_divisor);
		break;
	case SENSOR_HDR_EV_LEVE_1:
		ev_multiplier = 2;
		sp2509_increase_hdr_exposure(ev_multiplier);
		break;
 ...
 *============================================================================*/
static uint32_t sp2509_ext_func(unsigned long param)
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint32_t sp2509_stream_on(uint32_t param)
{
	SENSOR_PRINT("E");
   Sensor_WriteReg(0xfd, 0x01);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint32_t sp2509_stream_off(uint32_t param)
{
	SENSOR_PRINT("E");
