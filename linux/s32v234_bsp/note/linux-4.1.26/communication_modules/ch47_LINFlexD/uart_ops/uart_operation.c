...
ssize_t safe_write(int fd, const void *vptr, size_t n)
{
	...
	while (nleft > 0) {
		...
		nleft -= nwritten;
		ptr   += nwritten;
		printf("[%s]: write %d bytes, %d bytes left.\n", __func__, nwritten, nleft);
	}
	return(n);
}
...
ssize_t safe_read(int fd, void *vptr, size_t n)
{
	...
	while (nleft > 0) {
		...
		nleft -= nread;
		ptr += nread;
		printf("[%s]: read %d bytes, %d bytes left.\n", __func__, nread, nleft);
	}
	return (n-nleft);
}
...
int uart_read(int fd, char *r_buf, size_t len)
{
	...
	case 0:
		...
		return -1;
	default:
		fprintf(stderr, "safe_read-start!\n");
		cnt = safe_read(fd, r_buf, len);
		fprintf(stderr, "safe_read-end!\n");
		if (cnt == -1) {
