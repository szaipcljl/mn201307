...
static ssize_t xc7027_reg_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int ret = 0;
	struct roi_rec xc7027_roi_rec;
...
static DEVICE_ATTR(xc7027_reg, S_IRUGO | S_IWUSR, xc7027_reg_show, xc7027_reg_set);
#endif


#ifdef CAMERA_REG_DEBUG
static ssize_t camera_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	 return sprintf(buf, "\n\n\nPlease Input Command As Follows:\n" \
						 "echo x r r_p r_a > camera_reg\n" \
						 "echo x w r_p r_a r_v > camera_reg\n" \
						 "echo a r r_a > camera_reg\n" \
						 "echo a w r_a r_v > camera_reg\n" \
						 "x/a : x-xc7027 a-ar0144\n" \
						 "r/w : r-read   w-write\n"
						 "r_p : reg page\n"
						 "r_a : reg address\n"
						 "r_v : reg value\n\n");
...
static int camera_reg_parse_buf(const char *buf, u16 *reg_buf)
{
	u8 i = 0, j = 0, k = 0;
	u8 buf_len = 0;
	...
	for(i=0; i<buf_len-1; i++) {
		...
		for(j=0; j<8; j++) {
			...
			if((temp == 'x' || temp == 'a') && k == 0) {
				data[k] = temp;
				k++;
				break;
			}

			if((temp == 'r' || temp == 'w') && k ==1) {
				data[k] = temp;
				k++;
				break;
			}
			...
			//	num		0 		9		A		F		a		f
			//	ASCII	0x30	0x39	0x41	0x46	0x61	0x66
			if(temp >= 0x61 && temp <= 0x66) //a-f
				temp = temp - 0x57;
			else if(temp >= 0x41 && temp <= 0x46) //A-F
				temp = temp - 0x37;
			else if(temp >= 0x30 && temp <= 0x39) //0-9
				temp = temp - 0x30;
			else {
				printk("The Char of input parameters are invalid\n");
				return -2;
	...
	reg_buf[0] = data[0];  // x-a
	reg_buf[1] = data[1];  // r-w
	reg_buf[2] = data[2];  // xc7027->reg_page ar0144->reg_addr
	reg_buf[3] = data[3];  // xc7027->reg_addr ar0144->reg_val
	reg_buf[4] = data[4];  // xc7027->reg_val  ar0144->null

	return 0;
}

static void camera_xc7027_reg_read(u16 reg_page, u16 reg_addr)
{
	u8 reg_val = 0;
	u8 page = (u8)(reg_page & 0xFF);

	mutex_lock(&xc7027_mutex);
	xc_write_reg(0xfffd, 0x80);
	xc_write_reg(0xfffe, page);
	xc_read_reg(reg_addr, &reg_val);
	mutex_unlock(&xc7027_mutex);
	printk("READ : xc7027 page=0x%02x addr=0x%04x  val=0x%02x\n", page, reg_addr, reg_val);
}

static void camera_xc7027_reg_write(u16 reg_page, u16 reg_addr, u16 reg_val)
{
	u8 val = (u8)(reg_val & 0xFF);
	u8 page = (u8)(reg_page & 0xFF);

	mutex_lock(&xc7027_mutex);
	xc_write_reg(0xfffd, 0x80);
	xc_write_reg(0xfffe, page);
	xc_write_reg(reg_addr, val);
	mutex_unlock(&xc7027_mutex);
}

static void camera_ar0144_reg_read(u16 reg_addr)
{
	u16 reg_val = 0;

	mutex_lock(&xc7027_mutex);
	xc_write_regs(bypass_on_regs, ARRAY_SIZE(bypass_on_regs));
	ar_read_reg(AR0144_ADDR, reg_addr, &reg_val);
	xc_write_regs(bypass_off_regs, ARRAY_SIZE(bypass_off_regs));
	mutex_unlock(&xc7027_mutex);
	printk("READ : ar0144 addr=0x%04x val=0x%04x\n", reg_addr, reg_val);
}

static void camera_ar0144_reg_write(u16 reg_addr, u16 reg_val)
{
	mutex_lock(&xc7027_mutex);
	xc_write_regs(bypass_on_regs, ARRAY_SIZE(bypass_on_regs));
	ar_wrtie_reg(AR0144_ADDR, reg_addr, reg_val);
	xc_write_regs(bypass_off_regs, ARRAY_SIZE(bypass_off_regs));
	mutex_unlock(&xc7027_mutex);
}

static ssize_t camera_reg_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	u16 reg_buf[5] = {0};

	ret = camera_reg_parse_buf(buf, reg_buf);
	if(ret < 0)
		return count;

	if( reg_buf[0] == 'x' && reg_buf[1] == 'r')          // xc7027 read reg
		camera_xc7027_reg_read(reg_buf[2], reg_buf[3]);
	else if( reg_buf[0] == 'x' && reg_buf[1] == 'w')     // xc7027 write reg
		camera_xc7027_reg_write(reg_buf[2], reg_buf[3], reg_buf[4]);
	else if( reg_buf[0] == 'a' && reg_buf[1] == 'r')     // ar0144 read reg
		camera_ar0144_reg_read(reg_buf[2]);
	else if( reg_buf[0] == 'a' && reg_buf[1] == 'w')     // ar0144 write reg
		camera_ar0144_reg_write(reg_buf[2], reg_buf[3]);
	else
		printk("The Input Command Invalid!\n");
...
<<<<<<< HEAD
	...
	if(retval < 0) {
		dev_err(dev, "camera_reg device create file failed\n");
		return retval;
	}
#endif
...
>>>>>>> 8f8a302ea4de02a3e7c703853dd43e1fdbdcc534
	...
	power_on();
	dev_init();
	printk(PFLAG" xc7027_ar0144_data camera module is found\n");
