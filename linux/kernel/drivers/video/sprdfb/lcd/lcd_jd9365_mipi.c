...
{LCM_SEND(2),{0x55,0x90}},//151124 color
...
static LCM_Force_Cmd_Code rd_prep_code[]={
	...
	{0x37, {LCM_SEND(2), {0x2, 0}}}, //return two byte
};

static LCM_Force_Cmd_Code rd_prep_code_1[]={
	{0x37, {LCM_SEND(2), {0x1, 0}}}, //return one byte
};
static int32_t jd9365_mipi_init(struct panel_spec *self)
{
	...
	printk( "kernel  jd9365_mipi_init\n");
...
static uint32_t jd9365_readid(struct panel_spec *self)
{
	...
	printk("kernel lcd_jd9365_mipi read id!\n");
#if 0
	mipi_set_cmd_mode();
	mipi_eotp_set(1,0);
...
static int32_t jd9365_enter_sleep(struct panel_spec *self, uint8_t is_sleep)
{
	...
	printk("jd9365_enter_sleep, is_sleep = %d\n", is_sleep);
...
//	uint32_t reg_val_2 = 0;
	...
	pr_debug("lcd_jd9365_mipi read power mode!\n");
	mipi_eotp_set(0,1);
	...
	for(j = 0; j < 4; j++){
		...
		//printk("lcd_jd9365 mipi read power mode 0x0A value is 0x%x! , read result(%d)\n", read_data[0], read_rtn);

		if((0x9c == read_data[0])  && (0 == read_rtn)){
			pr_debug("lcd_jd9365_mipi read power mode success!\n");
			mipi_eotp_set(1,1);
			return 0x9c;
		}
	}
...
static uint32_t jd9365_check_esd(struct panel_spec *self)
{
	...
	if(power_mode == 0x9c){
		pr_debug("jd9365_check_esd OK!\n");
		return 1;
	}else{
		printk("jd9365_check_esd fail!(0x%x)\n", power_mode);
		return 0;
	}
}
static int32_t jd9365_after_suspend(struct panel_spec *self)
{
    // Do nothing;
...
//	.panel_after_suspend = jd9365_after_suspend,
}; 
