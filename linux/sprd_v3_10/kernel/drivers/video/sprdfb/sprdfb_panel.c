...
static uint32_t lcd_vol_from_uboot = 0;
static uint32_t lcd_mode_from_uboot = 0;
#endif
...
#ifdef CONFIG_LCD_COMPATIBLE_BY_READ_ID_VOLTAGE  //GST huafeizhou140324 add-s
static int __init lcd_vol_get(char *str)
{
	if ((str != NULL) && (str[0] == 'V') && (str[1] == 'T')) {
		sscanf(&str[2], "%x", &lcd_vol_from_uboot);
	}
	printk(KERN_INFO "sprdfb: [%s]LCD Panel voltage from uboot: 0x%x\n", __FUNCTION__, lcd_vol_from_uboot);
	return 1;
}
__setup("lcd_vol_type=", lcd_vol_get);
#endif
...
static ssize_t lcd_information_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	uint32_t lcd_id;

#ifdef CONFIG_LCD_COMPATIBLE_BY_READ_ID_VOLTAGE  //GST huafeizhou140324 add-s
    	return sprintf(buf, "Lcd ID: 0x%04X Vol Type:%d\n", lcd_id_from_uboot,lcd_vol_from_uboot);
#else
	//lcd_id=load_lcd_id_to_kernel();
    	return sprintf(buf, "Lcd ID: 0x%04X.\n", lcd_id_from_uboot);
#endif
}

static struct kobj_attribute lcd_information = {
    .attr = {
        .name = "lcd_information",
        .mode = S_IRUGO,
    },
    .show  = &lcd_information_show,
    .store = NULL,
};

static struct attribute *properties_attrs[] = {

    &lcd_information.attr,
    NULL
};
...
static void lcd_info_init(void)
{
    ...
    properties_kobj = kobject_create_and_add("board_properties2", NULL);
    if (properties_kobj)
...
int panel_init(struct sprdfb_device *dev)
{
	...
	#if 1 //huafeizhou141014 add-s
		lcd_info_init();//huafeizhou141014 add
	#endif
...
static struct panel_spec *adapt_panel_from_uboot(uint16_t dev_id)
{
	...
	list_for_each_entry(cfg, panel_list, list) {
		...
		#ifdef CONFIG_LCD_COMPATIBLE_BY_READ_ID_VOLTAGE  //GST huafeizhou140324 add-s
			printk(KERN_INFO "sprdfb: [%s]: LCD Voltage Type is  %d !! \n", __FUNCTION__,lcd_vol_from_uboot);
			if(NULL !=cfg->panel->ops->panel_get_vol_type)
				cfg->panel->ops->panel_get_vol_type(cfg->panel,lcd_vol_from_uboot);
		#endif
			return cfg->panel;
		}
	}
