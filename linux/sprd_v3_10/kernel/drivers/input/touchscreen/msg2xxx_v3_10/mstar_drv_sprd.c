...
int TOUCH_SCREEN_X_MAX = 480;
int TOUCH_SCREEN_Y_MAX = 800;
...
static struct msg_ts_platform_data *msg2xxx_ts_parse_dt(struct device *dev)
{
        ...
        const struct i2c_device_id *id)
{
	...
	if (np && !pdata)
	{
		...
		printk("*** %s(rst=%d,int=%d,x_max=%d,y_max=%d) ***\n", __FUNCTION__,MS_TS_MSG_IC_GPIO_RST,MS_TS_MSG_IC_GPIO_INT,TOUCH_SCREEN_X_MAX,TOUCH_SCREEN_Y_MAX);
	}
#endif
