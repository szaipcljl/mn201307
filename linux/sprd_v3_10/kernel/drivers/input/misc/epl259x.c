...
#include <linux/platform_device.h>
#ifdef CONFIG_INPUT_STATUS_HW
#include <linux/input/input_status.h>
#endif
/******************************************************************************
* configuration
*******************************************************************************/
...
#define PS_LOW_THRESHOLD		10000 //3000
#define PS_HIGH_THRESHOLD		15000 //4000
...
#endif
    err = request_irq(epld->irq,epl_sensor_eint_func, IRQF_TRIGGER_FALLING|IRQF_NO_SUSPEND,
                      "epl2182_pls", epld);
    if(err <0)
    {
...
static int epl_sensor_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
    ...
    if((i2c_smbus_read_byte_data(client, 0x21)) != 0x81){
		#ifdef CONFIG_INPUT_STATUS_HW
		input_status.pl = _STA_IDERRO;
		#endif
        LOG_INFO("elan ALS/PS sensor is failed. \n");
        goto i2c_fail;
     }
	#ifdef CONFIG_INPUT_STATUS_HW
	input_status.pl = _STA_IDOK;
	#endif
