...
{
	int ret =0 ;
	if(sprdbat_data->bat_info.ac_online == 1)
		ret=1;
	return ret;
}
					...
					const char *buf, size_t count)
{
	...
	case OTP_HIGH_STOP:
		...
		{
			local_irq_save(irq_flag);
			sprdbat_data->pdata->otp_high_stop = set_value-1000;
			local_irq_restore(irq_flag);
		}
		break;
	case OTP_HIGH_RESTART:
		if((set_value <=1650) && (set_value >=0))
		{
			local_irq_save(irq_flag);
			sprdbat_data->pdata->otp_high_restart = set_value-1000;
			local_irq_restore(irq_flag);
		}
		break;
	case OTP_LOW_STOP:
		...
		{
			local_irq_save(irq_flag);
			sprdbat_data->pdata->otp_low_stop = set_value-1000;
			local_irq_restore(irq_flag);
		}
		break;
	case OTP_LOW_RESTART:
		...
		{
			local_irq_save(irq_flag);
			sprdbat_data->pdata->otp_low_restart = set_value-1000;
			local_irq_restore(irq_flag);
		}
		break;
	//ntc end
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	case OTP_HIGH_STOP:
		...
		break;
	case OTP_HIGH_RESTART:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n", sprdbat_data->pdata->otp_high_restart+1000);
		break;
	case OTP_LOW_STOP:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n", sprdbat_data->pdata->otp_low_stop+1000);
		break;
	case OTP_LOW_RESTART:
