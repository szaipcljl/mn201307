...
#define HI544_EEPROM_SLAVE_ADDR 0x108
...
static  struct sensor_reg_tag hi544_common_init[] = {
		{0x0014, 0x42}, // B[7] Justification control (Data Justification on output Data)
		{0x0015, 0x0c}, // B[7] Tris,PCLK Inverting   0x00 2.0, 0x0c 3.0
		{0x0016, 0x00}, // MIPI Spec --> 0.9
		{0x0017, 0x2B}, // YUV422:0x18 RAW10:0x2B RAW8:0x2A RAW12:0x2C
		{0x0019, 0x04}, // Select Data_ID by REGISTER(0x0017)
		{0x0002, 0x19}, // B[7:2] ui_x4_clk_lane (Unit interval time step 0.25ns) 3ns  0.25 X 12
		{0x0003, 0x00}, // B[4] hs_invert_clk_lane (Invert HS signals)
		{0x0004, 0x02}, // B[4] hs_rx_term_e_subLVDS_clk_lane (Enables the hs-termination for subLVDs mode)
		{0x0005, 0x03}, // B[5] force_rx_mod_data_lane (Force lane module into receive mode wait for stop state)
		{0x0006, 0x01}, // B[5] Cd_off Contention_detector on/off
		{0x0009, 0x01}, // B[5] force_rx_mod_data_lane (Force lane module into receive mode wait for stop state)
		{0x000A, 0x01}, // B[5] Cd_off Contention_detector on/off
	 //////////////////////////////////////////////////
		...
		//Firmware 2Lane v0.44, LB OTP Burst On + double luma under dark. OTP Verify Mode. FW 20150327
		{0x2000, 0x4031},
		{0x2002, 0x83F8},
		{0x2004, 0x4104},
		{0x2006, 0x4307},
		...
		{0x0C0E, 0x2500}, //0x07 BLC display On, 0x05 BLC D off //BLC_ctl3. Frame BLC On = 0x05, off=0x04 //FBLC_ctl3. [0]en_fblc, [1]en_blc_bypass, [2]en_fblc_dpc, [5]en_fobp_dig_offset, [7]en_lobp_dpc_bypass
		{0x0C10, 0x0510}, //dig_blc_offset_h
		{0x0C16, 0x4040}, //fobp_dig_b_offset. red b[7] sign(0:+,1:-), b[6:0] dc offset 128(max)
		{0x0C18, 0x4040}, //fobp_dig_Gb_offset. Gr b[7] sign(0:+,1:-), b[6:0] dc offset 128(max)
		...
		{0x0000, 0x0000}, //orientation. [0]:x-flip, [1]:y-flip.
		...
		//-----< Exp.Time >------------------------//
		// Pclk_88.00Mhz @ Line_length_pclk : 2880 @Exp.Time 33.33ms
		{0x0004, 0x0729}, //coarse_int : 33.33ms@Pclk88mhz@Line_length2880
...
static void Sensor_PowerOn (uint32_t power_on)
{
	uint32_t mclk = 24;
	uint32_t un_mclk = 0;
	SENSOR_VDD_VAL_E dvdd_val = SENSOR_VDD_1200MV;
	SENSOR_VDD_VAL_E avdd_val = SENSOR_VDD_2800MV;
	SENSOR_VDD_VAL_E iovdd_val = SENSOR_VDD_1800MV;
	SENSOR_VDD_VAL_E vdd = SENSOR_VDD_CLOSED;
	uint32_t reset = 0;
	uint32_t pd = 0;
	uint32_t un_reset = 1;
	uint32_t un_pd = 1;
	if(1 == power_on) {
		sensor_set_mclk_fromkernel(pfile, un_mclk, 0);
		sensor_set_voltage_avdd_fromkernel(pfile, vdd, 0);
		sensor_set_voltage_dvdd_fromkernel(pfile, vdd, 0);
		sensor_set_voltage_iovdd_fromkernel(pfile, vdd, 0);
		sensor_set_rst_level_fromkernel(pfile, reset, 0);
		sensor_set_pd_level_fromkernel(pfile, pd, 0);
		msleep(20);

		sensor_set_voltage_iovdd_fromkernel(pfile, iovdd_val, 0);
		msleep(2);
		sensor_set_voltage_avdd_fromkernel(pfile, avdd_val, 0);
		msleep(2);
		sensor_set_voltage_dvdd_fromkernel(pfile, dvdd_val, 0);
		msleep(2);
		sensor_set_pd_level_fromkernel(pfile, un_pd, 0);
		msleep(2);
		sensor_set_mclk_fromkernel(pfile, mclk, 0);
		msleep(15);
		sensor_set_rst_level_fromkernel(pfile, un_reset, 0);
		msleep(700);
	}else {
		msleep(4);
		sensor_set_rst_level_fromkernel(pfile, reset, 0);
...
static int Sensor_init(struct sensor_reg_tag *reg_ptr, uint32_t reg_count)
{
	int ret = SENSOR_SUCCESS;
	struct sensor_reg_tab_tag regTab;
	regTab.sensor_reg_tab_ptr = reg_ptr;
	regTab.reg_count = reg_count;
	regTab.burst_mode = 0;
	regTab.reg_bits = (0x01 << 1) | 0x01 | (0x04 << 5);
	ret = sensor_wr_regtab_fromkernel(pfile,  &regTab, 0);

	return ret;
}

static uint16_t Sensor_ReadReg(uint16_t reg_addr)
{
	int ret = SENSOR_SUCCESS;
	struct sensor_reg_bits_tag reg;
	reg.reg_addr = reg_addr;
	reg.reg_bits = (0x01 << 1) | 0x01 | (0x04 << 5);
	ret = sensor_ReadReg_fromkernel(pfile, &reg);
...
static int Sensor_WriteI2C (uint16_t slave_addr, uint8_t * cmd, uint16_t cmd_length)
{
	...
	sensor_wr_i2c_fromkernel(pfile, &i2c_tab);
	return ret;
}
...
static int Sensor_ReadI2C (uint16_t slave_addr, uint8_t * cmd, uint16_t cmd_length)
{
	...
	sensor_rd_i2c_fromkernel(pfile, &i2c_tab, 0);
	return ret;
}

static void read_otp_over(void)
{
	...
	cmd_val[1] = 0x18;
	cmd_val[2] = 0x00;
	Sensor_WriteI2C(hi544_I2C_ADDR_W,(uint8_t*)&cmd_val[0], cmd_len);
	cmd_val[0] = 0x00;
	cmd_val[1] = 0x3E;
	cmd_val[2] = 0x00;
	Sensor_WriteI2C(hi544_I2C_ADDR_W,(uint8_t*)&cmd_val[0], cmd_len);
...
static uint32_t read_otp (void *sensor_handle, int sensor_id, int cmd, long arg)
{
	...
	MM_TRACE ("SENSOR_hi544: hi544_read_otp E\n");
	...
	cmd_val[0] = 0x01;
	cmd_val[1] = 0x0A;
	cmd_val[2] = ((start_addr) >>8)&0xff;
	Sensor_WriteI2C(hi544_I2C_ADDR_W,(uint8_t*)&cmd_val[0], cmd_len);
	cmd_val[0] = 0x01;
	cmd_val[1] = 0x0B;
	cmd_val[2] = (start_addr)&0xff;
	Sensor_WriteI2C(hi544_I2C_ADDR_W,(uint8_t*)&cmd_val[0], cmd_len);
	cmd_val[0] = 0x01;
	cmd_val[1] = 0x02;
	cmd_val[2] = 0x01;
	Sensor_WriteI2C(hi544_I2C_ADDR_W,(uint8_t*)&cmd_val[0], cmd_len);
	for (i=0; i<len; i++) {
		tmp = Sensor_ReadReg(0x108); // OTP data read
		buff[i] = (tmp>>8)&0xff;
	}
	param_ptr->len = len;

	MM_TRACE("SENSOR_hi544: hi544_read_otp X\n");
	return rtn;
}
...
int sensor_reloadinfo_thread (void *data)
{
	...
	struct sensor_reg_tag *reg_ptr = hi544_common_init;
	uint32_t reg_count = sizeof(hi544_common_init) / sizeof(hi544_common_init[0]);
	uint16_t i2c_addr = (hi544_I2C_ADDR_R & 0xff);
	...
	while(1){
		...
		if (IS_ERR (pfile)) {
			...
			MM_TRACE ("open /dev/sprd_sensor OK\n");
			break;
		}
	}

	Sensor_PowerOn (1);
	sensor_k_set_i2c_addr(pfile, i2c_addr);
	if (0 != ret) {
		MM_TRACE ("set sensor i2c addr failed\n");
		goto EXIT;
    }
	ret = sensor_set_i2c_clk_fromkernel(pfile, clock, 0);
	if (0 != ret) {
		MM_TRACE ("set sensor i2c clock failed\n");
		goto EXIT;
    }
	ret = Sensor_init(reg_ptr, reg_count);
	if (0 != ret) {
		MM_TRACE ("sensor init failed\n");
		goto EXIT;
    }
  ...
  //checksum
	...
	val.pval = param_ptr;
	ret = read_otp (NULL, 0, SENSOR_ACCESS_VAL, (long) &val);
