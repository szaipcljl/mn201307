...
static int write_reg(struct gf_dev* gf_dev, u16 addr, u8 value)
{
    int status = 0;
...
static int read_reg(struct gf_dev* gf_dev, u16 addr, u8* value)
{
    int status = 0;
    mutex_lock(&gf_dev->buf_lock);
...
static void read_4k_bytes(struct gf_dev* gf_dev, u16 addr, u8* buf)
{
    mutex_lock(&gf_dev->buf_lock);
    gf_spi_read_bytes(gf_dev, addr, 2048, gf_dev->buffer);
    memcpy(buf, gf_dev->buffer+GF_RDATA_OFFSET, 2048);
    gf_spi_read_bytes(gf_dev, addr+2048, 2048, gf_dev->buffer);
    memcpy(buf+2048, gf_dev->buffer+GF_RDATA_OFFSET, 2048);
    mutex_unlock(&gf_dev->buf_lock);
}
static void write_4k_bytes(struct gf_dev* gf_dev, u16 addr, u8* buf)
{
    mutex_lock(&gf_dev->buf_lock);
    memcpy(gf_dev->buffer+GF_WDATA_OFFSET, buf, 2048);
    gf_spi_write_bytes(gf_dev, addr, 2048, gf_dev->buffer);
    memcpy(gf_dev->buffer+GF_WDATA_OFFSET, buf+2048, 2048);
    gf_spi_write_bytes(gf_dev, addr+2048, 2048, gf_dev->buffer);
    mutex_unlock(&gf_dev->buf_lock);
}
 ...
 * */
#if 1
static int is_init_complete(struct gf_dev* gf_dev)
{
    int time_out = 0;
 ...
 * */
static int is_burn_complete(struct gf_dev* gf_dev)
{
    int time_out = 0;
...
/*
   static void update_init_9p(struct gf_dev *gf_dev)
   {
 ...
 */
void gf_run_ice(struct gf_dev* gf_dev)
{
    // update_init_9p(gf_dev);

    //0x2676->0x56,0x5d,0x08,0x1D
    write_reg(gf_dev, 0x4180, 0x0C);

    write_reg(gf_dev, 0x404C, 0x01); /*<DSP EN>*/
    write_reg(gf_dev, 0x404D, 0x01); /*<patch0 EN>*/
    write_reg(gf_dev, 0x404E, 0x01); /*<patch1 EN>*/

    write_reg(gf_dev, 0x4278, 0x01); /*<Flash Power-On,default:0>*/
    write_reg(gf_dev, 0x422E, 0x01); /*<Flash IO_EN,default:0>*/
    write_reg(gf_dev, 0x422C, 0x01); /*<Flash CTRL_EN,default:1>*/
    write_reg(gf_dev, 0x422D, 0x00); /*<Flash CTRL BY MicroP,default:0>*/

    /*<ICE ADDR0>*/
    write_reg(gf_dev, 0x42C4, 0x76);//use 20MHz clock
    write_reg(gf_dev, 0x42C5, 0x26);
    write_reg(gf_dev, 0x42C6, 0x76);
    write_reg(gf_dev, 0x42C7, 0x26);

    /*<ICE ADDR1>*/
    write_reg(gf_dev, 0x42C8, 0x2F);//flashless flag set to 0
    write_reg(gf_dev, 0x42C9, 0x2A);
    write_reg(gf_dev, 0x42CA, 0x2F);
    write_reg(gf_dev, 0x42CB, 0x2A);

    /*<ICE ADDR2>*/
    write_reg(gf_dev, 0x42CC, 0x30);//debug flag 0xA5
    write_reg(gf_dev, 0x42CD, 0x00);
    write_reg(gf_dev, 0x42CE, 0x30);
    write_reg(gf_dev, 0x42CF, 0x00);

    /*<ICE DATA0>*/
    write_reg(gf_dev, 0x42E4, 0x56);
    write_reg(gf_dev, 0x42E5, 0x5D);
    write_reg(gf_dev, 0x42E6, 0x08);
    write_reg(gf_dev, 0x42E7, 0x1D);

    /*<ICE DATA1>*/
    write_reg(gf_dev, 0x42E8, 0x33);//clock = 20M
    write_reg(gf_dev, 0x42E9, 0x21);
    write_reg(gf_dev, 0x42EA, 0x5B);
    write_reg(gf_dev, 0x42EB, 0x62);

    /*<ICE DATA2>*/
    write_reg(gf_dev, 0x42EC, 0x18);//debug flag 0x5094 = 0xA5
    write_reg(gf_dev, 0x42ED, 0x7B);
    write_reg(gf_dev, 0x42EE, 0xA9);
    write_reg(gf_dev, 0x42EF, 0x0A);

    write_reg(gf_dev, 0x4304, 0x07);/*ICE ADDR EN*/
    write_reg(gf_dev, 0x4305, 0x07);

    write_reg(gf_dev, 0x404F, 0x00);
    write_reg(gf_dev, 0x4180, 0x08);//Release CPU and hold DSP
}
 ...
 */
static int download_dsp_isp_9p(struct gf_dev* gf_dev, unsigned char *buf, u16 len)
{
    /*1.Close watch-dog,  clear cache enable(write 0 to 0x40B0)*/
    write_reg(gf_dev, 0x40B0, 0x0);
    write_reg(gf_dev, 0x404B, 0x0);

    /*Confg to boot from SRAM(write 0x2 to 0x4190)*/
    write_reg(gf_dev, 0x4190, 0x2);

    /*3.software reset(write 0x1 to 0x4184)*/
    write_reg(gf_dev, 0x4184, 0x01);

    /* 4.Enable bank3(write 0x3 to 0x4048)*/
    write_reg(gf_dev, 0x4048, 0x03);

    /*5.Enable accessing code(write 0x1 to 0x404c)*/
    write_reg(gf_dev, 0x404C, 0x01);

    /*6.Burn the DSP ISP code to the area begin from 0xC000*/
    // write_data(0xC000, len, buf);
    write_4k_bytes(gf_dev, 0xC000, buf);
    ...
    /* 7.Set scramble(write 0x0 to 0x4218)*/
    write_reg(gf_dev, 0x4218, 0x00);

    /*8.Disable accessing code (writ 0 to 0x404C)*/
    write_reg(gf_dev, 0x404C, 0x00);
 ...
 * */
static int update_patch_block0(struct gf_dev* gf_dev, unsigned char *buf, unsigned short len)
{
    ...
    write_reg(gf_dev, 0x4180, 0x0C);
    /*2.Clean control flag(write 0x0 to 0x5094)*/
    write_reg(gf_dev, 0x5094, 0x00);
    /*3.Enbale bank4(write 0x4 to 0x4048)*/
    write_reg(gf_dev, 0x4048, 0x04);
    /*4.Enable accessing code(write 0x1 to 0x404d)*/
    write_reg(gf_dev, 0x404D, 0x01);
    ...
    // write_data(0xC000, len, buf);
    write_4k_bytes(gf_dev, 0xC000, buf);
    write_4k_bytes(gf_dev, 0xC000+4096, buf+4096);
 ...
 * */
static int update_patch_block1(struct gf_dev* gf_dev, unsigned char *buf, unsigned short len)
{
    /* 12.Hold SS51 and DSP(write 0x0C to 0x4180)*/
    write_reg(gf_dev, 0x4180, 0x0C);
    /* 13.Enbale bank5(write 0x5 to 0x4048)*/
    write_reg(gf_dev, 0x4048, 0x05);
    /*14.Enable accessing code(write 0x1 to 0x404d)*/
    write_reg(gf_dev, 0x404E, 0x01);
 ...
 * 21.Disable accessing code(write 0x00 to 0x404E)
 * */
static int update_patch_block2(struct gf_dev* gf_dev, unsigned char *buf, unsigned short len)
{
    /* 12.Hold SS51 and DSP(write 0x0C to 0x4180)*/
    write_reg(gf_dev, 0x4180, 0x0C);
    /* 13.Enbale bank5(write 0x5 to 0x4048)*/
 ...
 * 21.Disable accessing code(write 0x00 to 0x404E)
 * */
static int update_patch_block3(struct gf_dev* gf_dev, unsigned char *buf, unsigned short len)
{
    /* 12.Hold SS51 and DSP(write 0x0C to 0x4180)*/
    write_reg(gf_dev, 0x4180, 0x0C);
    /* 13.Enbale bank5(write 0x5 to 0x4048)*/
 ...
 * */
static int update_dsp_code_9p(struct gf_dev* gf_dev, unsigned char *buf, unsigned short len)
{
    /* 1.Select bank3(write 0x3 to 0x4048)*/
    write_reg(gf_dev, 0x4048, 0x03);
    /* 2.Hold SS51 and DSP(write 0x0C to 0x4180)*/
    ...
    write_4k_bytes(gf_dev, 0x9000, buf);
    mdelay(1);
...
int gf_fw_update(struct gf_dev* gf_dev, unsigned char *buf, unsigned short len)
{
    ...
    pr_info("Update patch code part0.\n");
    p_fw = buf + 14;
    len = 8192;
    ...
    pr_info("Update patch code part1.\n");
    p_fw = buf + 8192 + 14;
    len = 8192;
    ...
    pr_info("Update patch code part2.\n");
    p_fw = buf + 8192*2 + 14;
    len = 8192;
    ...
    pr_info("Update patch code part3.\n");
    p_fw = buf + 8192*3 + 14;
    len = 8192;
