 ...
 * V4.0
 */
#define LOG_TAG "sensor_gc030a_pike"
...
#include "sensor_gc030a_raw_param_main.c"
#else
#include "sensor_gc030a_raw_param.c"
#endif
...
#define MODULE_ID_gc030a_yyy		0x0001    //xxx: sensor P/N;  yyy: module vendor
...
#include "sensor_gc030a_yyy_otp.c"

struct raw_param_info_tab s_gc030a_raw_param_tab[] = {
	{MODULE_ID_gc030a_yyy, &s_gc030a_mipi_raw_info, gc030a_yyy_identify_otp, gc030a_yyy_update_otp},
...
static const SENSOR_REG_T s_gc030a_preview_size_video_tab[SENSOR_VIDEO_MODE_MAX][1] = {
	/*video mode 0: ?fps */
	{
	 {0xffff, 0xff}
	 },
...
static const SENSOR_REG_T s_gc030a_capture_size_video_tab[SENSOR_VIDEO_MODE_MAX][1] = {
	/*video mode 0: ?fps */
	{
	 {0xffff, 0xff}
	 },
...
#define param_update(x1,x2) sprintf(name,"/data/gc030a_%s.bin",x1);\
...
static uint32_t gc030a_InitRawTuneInfo(void)
{
	uint32_t rtn=0x00;

	isp_raw_para_update_from_file(&g_gc030a_mipi_raw_info,0);

	struct sensor_raw_info* raw_sensor_ptr=s_gc030a_mipi_raw_info_ptr;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static int gc030a_get_otp_info(struct otp_info_t *otp_info)
{
	uint32_t ret = SENSOR_FAIL;
	uint32_t i = 0x00;

	//identify otp information
	for (i = 0; i < NUMBER_OF_ARRAY(s_gc030a_raw_param_tab); i++) {
		SENSOR_PRINT("identify module_id=0x%x",s_gc030a_raw_param_tab[i].param_id);

		if(PNULL!=s_gc030a_raw_param_tab[i].identify_otp){
			//set default value;
			memset(otp_info, 0x00, sizeof(struct otp_info_t));

			if(SENSOR_SUCCESS==s_gc030a_raw_param_tab[i].identify_otp(otp_info)){
				if (s_gc030a_raw_param_tab[i].param_id== otp_info->module_id) {
					SENSOR_PRINT("identify otp sucess! module_id=0x%x",s_gc030a_raw_param_tab[i].param_id);
					ret = SENSOR_SUCCESS;
					break;
				}
				else{
					SENSOR_PRINT("identify module_id failed! table module_id=0x%x, otp module_id=0x%x",s_gc030a_raw_param_tab[i].param_id,otp_info->module_id);
				}
			}
			else{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint32_t gc030a_apply_otp(struct otp_info_t *otp_info, int id)
{
	uint32_t ret = SENSOR_FAIL;
	//apply otp parameters
	SENSOR_PRINT("otp_table_id = %d", id);
	if (PNULL != s_gc030a_raw_param_tab[id].cfg_otp) {

		if(SENSOR_SUCCESS==s_gc030a_raw_param_tab[id].cfg_otp(otp_info)){
			SENSOR_PRINT("apply otp parameters sucess! module_id=0x%x",s_gc030a_raw_param_tab[id].param_id);
			ret = SENSOR_SUCCESS;
		}
		else{
 ...
 * please modify this function acording your spec
 *============================================================================*/
static uint32_t gc030a_cfg_otp(uint32_t param)
{
	uint32_t ret = SENSOR_FAIL;
	struct otp_info_t otp_info={0x00};
	int table_id = 0;

	table_id = gc030a_get_otp_info(&otp_info);
	if (-1 != table_id)
		ret = gc030a_apply_otp(&otp_info, table_id);
