				                     ...
				                     struct setting_cmd_parameter *parm)
{
	...
	if (NULL != p_exif_info->primary.img_desc_ptr) {
		...
		memset(property,'\0',sizeof(property));
		property_get("gst.camera.model.name", property, EXIF_DEF_MODEL);//ro.product.model GST_guojiangping modify 20151225
					                   ...
					                   struct setting_cmd_parameter *parm)
{
	...
	if (hal_param->is_update_range_fps) {
			...
			struct setting_local_param *local_param = get_local_param(cpt, parm->camera_id);
			CMR_LOGI("get dv mode %d",local_param->is_dv_mode_for_yuv);
