...
#if defined (CONFIG_CAMERA_FACE_DETECT) && defined (CONFIG_CAMERA_FACE_DETECT_FD)
...
typedef int (*face_finder_function)(unsigned char *src, struct face_finder_data **ppDstFaces, int *pDstFaceNum ,int skip, int smile);
...
static unsigned int face_detect_frame_num = 1;
static int latest_smile_level = 1;
				...
				cmr_handle *out_class_handle)
{
	...
	face_detect_frame_num = 1;
	latest_smile_level = 1;
...
static cmr_int fd_thread_proc(struct cmr_msg *message, void *private_data)
{
	...
	case CMR_EVT_FD_START:
		...
		unsigned char isneedsmilelevel = 0;
		if(face_detect_frame_num%5==0){
			isneedsmilelevel=1;
		}
		face_detect_frame_num++;
		...
		if (FD_FAIL == facesolid_ret) {
			...
			for (k = 0; k < min_fd; k++) {
				...
				CMR_LOGI("face detect sx = %d, sy = %d, ex = %d, ey = %d,smile_level:%d,smile_conf:%d",
				face_rect_ptr->sx, face_rect_ptr->sy, face_rect_ptr->ex, face_rect_ptr->ey,face_rect_ptr->smile_level,face_rect_ptr->smile_conf);
				if(isneedsmilelevel == 1){
					/*get the real smile level from libface so.we need store the max smile level for smile takepicture*/
					latest_smile_level = ((face_rect_ptr->smile_level)>latest_smile_level ) ?  face_rect_ptr->smile_level : latest_smile_level ;
				}else{
					face_rect_ptr->smile_level = latest_smile_level;
				}
				if(face_rect_ptr->smile_level <= 0)
					face_rect_ptr->smile_level = 1;
