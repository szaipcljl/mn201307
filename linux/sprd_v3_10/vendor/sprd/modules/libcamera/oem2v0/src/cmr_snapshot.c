...
cmr_int snp_proc_copy_frame(cmr_handle snp_handle, void *data)
{
	...
	if (1 != cxt->req_param.is_video_snapshot && 1 != cxt->req_param.is_zsl_snapshot) {
		...
		if (IMG_DATA_TYPE_RAW == chn_data_ptr->fmt) {
			...
			CMR_LOGI("raw y src_vir = %lx dst_vir = %lx width %d height %d channel_zoom_mode %ld",
				src_vir, dst_vir, width, height, proc_param_ptr->channel_zoom_mode);
			CMR_LOGI("raw_pixel_width=%d", raw_pixel_width);
			cmr_copy((void *)dst_vir, (void *)src_vir, width * height * raw_pixel_width / 8);
		} else {
			src_vir = (unsigned long)chn_data.yaddr_vir;
			dst_vir = (unsigned long)cxt->req_param.post_proc_setting.chn_out_frm[0].addr_vir.addr_y;
			...
			CMR_LOGI("uv src_vir = %lx dst_vir = %lx width %d height %d act_width %d act_height %d",
						   ...
						   struct cmr_op_mean *mean)
{
	...
	for (i=0; i<padding_lines; i++) {
		CMR_LOGI("dst_y_buf 0x%x last_y_buf 0x%x", dst_y_buf, last_y_buf);
	...
	for (i=0; i<padding_lines; i++) {
		CMR_LOGI("dst_uv_buf 0x%x last_uv_buf 0x%x", dst_uv_buf, last_uv_buf);
...
cmr_int snp_start_isp_proc(cmr_handle snp_handle, void *data)
{
	...
	if (snp_cxt->ops.raw_proc) {
		...
		if ((CAMERA_ISP_TUNING_MODE == snp_cxt->req_param.mode) || (CAMERA_ISP_SIMULATION_MODE == snp_cxt->req_param.mode)) {
...
cmr_int snp_update_postproc_src_size(cmr_handle snp_handle, struct img_frm *chn_data)
{
	...
	if (ZOOM_POST_PROCESS == proc_param_ptr->channel_zoom_mode) {
		chn_data->addr_vir.addr_u = (unsigned long)chn_data->addr_vir.addr_y + width * height;
		chn_data->addr_phy.addr_u = (unsigned long)chn_data->addr_phy.addr_y + width * height;
	} else {
		chn_data->addr_vir.addr_u= (unsigned long)chn_data->addr_vir.addr_y + act_width * act_height;
		chn_data->addr_phy.addr_u= (unsigned long)chn_data->addr_phy.addr_y + act_width * act_height;
	}
...
cmr_int snp_update_scale_param(cmr_handle snp_handle, struct img_frm chn_data)
{
	...
	for (i=0 ; i<CMR_CAPTURE_MEM_SUM ; i++) {
		...
		chn_param_ptr->scale[i].src_img.addr_phy.addr_u = chn_data.addr_phy.addr_u + (offset[i] >> 1);
		chn_param_ptr->scale[i].src_img.addr_phy.addr_v = chn_data.addr_phy.addr_v + (offset[i] >> 1);
	...
	case SNAPSHOT_EVT_CHANNEL_DONE:
		...
		if (1 == cxt->req_param.is_hdr) {
			...
			if (1 == cxt->req_param.is_video_snapshot || 1 == cxt->req_param.is_zsl_snapshot) {
				...
				src_vir = (unsigned long)chn_data.yaddr_vir;
				dst_vir = (unsigned long)cxt->req_param.post_proc_setting.chn_out_frm[0].addr_vir.addr_y;
				CMR_LOGI("y src_vir = %lx dst_vir = %lx width %d height %d channel_zoom_mode %d",
					src_vir, dst_vir, width, height, proc_param_ptr->channel_zoom_mode);
				cmr_copy((void *)dst_vir, (void *)src_vir, width * height);
				...
				CMR_LOGI("uv src_vir = %lx dst_vir = %lx width %d height %d act_width %d act_height %d max_size.width %d max_size.height %d",
					src_vir, dst_vir, width, height, act_width, act_height, proc_param_ptr->max_size.width, proc_param_ptr->max_size.height);
