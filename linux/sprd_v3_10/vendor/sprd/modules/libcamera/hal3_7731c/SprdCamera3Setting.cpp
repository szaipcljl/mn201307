...
const int32_t kavailable_back_stream_configurations[60][4] = {
	...
	*/
	...
	{HAL_PIXEL_FORMAT_YCbCr_420_888, 3771, 2121, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
...
#elif defined(CONFIG_CAMERA_SUPPORT_8M)
	{HAL_PIXEL_FORMAT_BLOB, 3771, 2121, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
	{HAL_PIXEL_FORMAT_BLOB, 3264, 2448, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
	{HAL_PIXEL_FORMAT_BLOB, 2592, 1944, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
//	{HAL_PIXEL_FORMAT_BLOB, 2560, 1536, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
	{HAL_PIXEL_FORMAT_BLOB, 2048, 1536, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
...
#elif defined(CONFIG_CAMERA_SUPPORT_5M)
	{HAL_PIXEL_FORMAT_BLOB, 2592, 1944, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
	{HAL_PIXEL_FORMAT_BLOB, 2592, 1458, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
//	{HAL_PIXEL_FORMAT_BLOB, 2560, 1536, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
	{HAL_PIXEL_FORMAT_BLOB, 2048, 1536, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
	{HAL_PIXEL_FORMAT_BLOB, 1920, 1088, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
	{HAL_PIXEL_FORMAT_BLOB, 1600, 1200, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
...
const int32_t kavailable_front_stream_configurations[60][4] = {
	...
	{HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED, 2048, 1536, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
#endif
#ifdef CONFIG_FRONT_CAMERA_SUPPORT_2M // add
	{HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED, 1920, 1088, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
	{HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED, 864, 480, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
#endif
...
#ifdef CONFIG_FRONT_CAMERA_SUPPORT_2M // add
	{HAL_PIXEL_FORMAT_BLOB, 1920, 1088, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
	{HAL_PIXEL_FORMAT_BLOB, 864, 480, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
#endif
#ifdef CONFIG_FRONT_CAMERA_SUPPORT_1080P
	{HAL_PIXEL_FORMAT_BLOB, 1920, 1088, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT},
#endif
...
int SprdCamera3Setting::GetFovParam(int32_t cameraId)
{
	...
	if (0 == cameraId) {
		...
		LOGI("cameraId is 2check front sensor numbers %d", SnNum);
		for(i=0; i<SnNum; i++) {
			if( strcmp(third_sensor_fov_tab[i].sn_name, CAMERA_SENSOR_TYPE_DEV_2) == 0) {
				LOGI("sensor matched the %dth  is %s", i, CAMERA_SENSOR_TYPE_DEV_2);
				retValue = i;
				break;
			}
		}
										...
										camera_metadata_t **metadata)
{
		...
		if(hasFocuser == true) {
			...
			if(type == CAMERA3_TEMPLATE_PREVIEW
				...
				if (characteristicsInfo.exists(ANDROID_CONTROL_AF_AVAILABLE_MODES)) {
					int32_t count = characteristicsInfo.find(ANDROID_CONTROL_AF_AVAILABLE_MODES).count;
					for (i=0; i < count; i++) {
			...
			if(characteristicsInfo.exists(ANDROID_REQUEST_AVAILABLE_REQUEST_KEYS)) {
				int32_t count = characteristicsInfo.find(ANDROID_REQUEST_AVAILABLE_REQUEST_KEYS).count;
				for (i=0; i < count; i++) {
	...
	{//check fps range
		if(characteristicsInfo.exists(ANDROID_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES)) {
			int32_t count = characteristicsInfo.find(ANDROID_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES).count;
			...
			aeTargetFpsRange[0] = characteristicsInfo.find(ANDROID_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES).data.i32[count-2];
			aeTargetFpsRange[1] = characteristicsInfo.find(ANDROID_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES).data.i32[count-1];

			if(type != CAMERA3_TEMPLATE_MANUAL
				&& type != CAMERA3_TEMPLATE_STILL_CAPTURE) {
				if(aeTargetFpsRange[1] < 20) {
					HAL_LOGE("max fps should be at least 20");
					return -1;
				}
	...
	{//check anti banding mode
		if(type != CAMERA3_TEMPLATE_MANUAL) {
			if(characteristicsInfo.exists(ANDROID_CONTROL_AE_AVAILABLE_ANTIBANDING_MODES)) {
				int32_t count = characteristicsInfo.find(ANDROID_CONTROL_AE_AVAILABLE_ANTIBANDING_MODES).count;
	...
	//sensor setting
	{
		...
		if(characteristicsInfo.exists(ANDROID_REQUEST_AVAILABLE_CHARACTERISTICS_KEYS)) {
			int32_t count = characteristicsInfo.find(ANDROID_REQUEST_AVAILABLE_CHARACTERISTICS_KEYS).count;
			for (i=0; i < count; i++) {
		...
		if(lens_aperture_capabilities == true) {
			...
			int32_t count = characteristicsInfo.find(ANDROID_LENS_INFO_AVAILABLE_APERTURES).count;
			//if(count > 1) {
...

	{
		...
		if(lens_filter_capabilities == true) {
			...
			int32_t count = characteristicsInfo.find(ANDROID_LENS_INFO_AVAILABLE_FILTER_DENSITIES).count;
			//if(count > 1) {
...

	{
		...
		if(characteristicsInfo.exists(ANDROID_LENS_INFO_AVAILABLE_FOCAL_LENGTHS)) {
			int32_t count = characteristicsInfo.find(ANDROID_LENS_INFO_AVAILABLE_FOCAL_LENGTHS).count;
			//if(count > 1) {
...

	{
		...
		if(lens_optical_capabilities == true) {
			...
			int32_t count = characteristicsInfo.find(ANDROID_LENS_INFO_AVAILABLE_OPTICAL_STABILIZATION).count;
			//if(count > 1) {
...

	{
		...
		if(characteristicsInfo.exists(ANDROID_REQUEST_AVAILABLE_CAPABILITIES)) {
			int32_t count = characteristicsInfo.find(ANDROID_REQUEST_AVAILABLE_CAPABILITIES).count;
			for (i=0; i < count; i++) {
	...
	if(type == CAMERA3_TEMPLATE_STILL_CAPTURE) {
...

		{
			...
			if(edge_mode_request == true) {
				...
				int32_t count = characteristicsInfo.find(ANDROID_EDGE_AVAILABLE_EDGE_MODES).count;
				for (i=0; i < count; i++) {
...

		{
			...
			if(noise_reduction_capabilities == true) {
				...
				int32_t count = characteristicsInfo.find(ANDROID_NOISE_REDUCTION_AVAILABLE_NOISE_REDUCTION_MODES).count;
				for (i=0; i < count; i++) {
...

		{
			...
			if(tone_map_capabilities == true) {
				...
				int32_t count = characteristicsInfo.find(ANDROID_TONEMAP_AVAILABLE_TONE_MAP_MODES).count;
				for (i=0; i < count; i++) {
...
void SprdCamera3Setting::resetFACEInfo()
{
	FACE_Tag *faceDetectionInfo =
		(FACE_Tag *)&(s_setting[mCameraId].faceInfo);

	HAL_LOGD("resetFACEInfo(), camera id: %d", mCameraId);
