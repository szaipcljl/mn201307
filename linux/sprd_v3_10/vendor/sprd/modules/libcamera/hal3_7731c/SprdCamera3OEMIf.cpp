...
void SprdCamera3OEMIf::shakeTestInit(ShakeTest *tmpShakeTest)
{
	...
	if((0 == strcmp("1", is_performance_camera_test)) && mIsPerformanceTestable) {
		...
		HAL_LOGD("SHAKE_TEST not come in");
...
#ifdef CONFIG_SPRD_PRIVATE_ZSL//fix bug 537051
...
//#ifndef CONFIG_SPRD_PRIVATE_ZSL//fix bug 537051
...
int SprdCamera3OEMIf::set_ddr_freq(uint32_t mhzVal)
{
	...
	HAL_LOGD("to %d now count %d freq %d E", mhzVal, mSetDDRFreqCount, mSetDDRFreq);
	...
	if (NULL == fp) {
		HAL_LOGW("failed to open %s X", set_freq);
		return BAD_VALUE;
	}
...
#ifdef CONFIG_SPRD_PRIVATE_ZSL
	SET_PARM(mCameraHandle, CAMERA_PARAM_SPRD_ZSL_ENABLED, (cmr_uint)mSprdZslEnabled);
#endif
...
//#else
		PushFirstVideobuff(); //fix bug 537051
//#endif
		if (!is_push_zsl)
			PushFirstZslbuff();
	}
...
void SprdCamera3OEMIf::receivePreviewFrame(struct camera_frame_type *frame)
{
	...
	if( skinWhitenLevel > 0 ){
		...
		if(faceInfo.face_num>0){
			...
			HAL_LOGD("UCAM update rect:%d-%d-%d-%d",mSkinWhitenTsface.left,mSkinWhitenTsface.top,mSkinWhitenTsface.right,mSkinWhitenTsface.bottom);
		}
		mShinWhitenNotDetectFDNum++;
#define SHINWHITED_NOT_DETECTFD_MAXNUM 30
		if(mShinWhitenNotDetectFDNum>SHINWHITED_NOT_DETECTFD_MAXNUM){
		...
		if (frame->width > 0 && frame->height > 0) {
			...
			if(mu_retVal !=  TS_OK) {
				HAL_LOGD("UCAM ts_face_beautify ret is %d.rect:%d-%d-%d-%d", ret,mSkinWhitenTsface.left,mSkinWhitenTsface.top,mSkinWhitenTsface.right,mSkinWhitenTsface.bottom);
			} else {
	...
	if(channel) {
		...
		if(pre_stream) {//preview stream
			...
			if(skinWhitenLevel != 0){
				...
				int fd = private_handle->share_fd;
				MemoryHeapIon::Flush_ion_buffer(fd,(void*)frame->y_vir_addr,(void*)frame->y_phy_addr,frame->width*frame->height*3/2);
			}
			if(ret == NO_ERROR) {
											...
											void* parm4)
{
	...
	case CAMERA_RSP_CB_SUCCESS:
	{
		HAL_LOGD("CAMERA_RSP_CB_SUCCESS");
	...
	{
		HAL_LOGD("CAMERA_EVT_CB_CAPTURE_FRAME_DONE");
		JPEG_Tag jpegInfo;
		...
		if(jpeg_gps_location)
		{
			...
			HAL_LOGD("gps pt.latitude = %f, pt.altitude = %f, pt.longitude = %f, pt.process_method = %s, jpegInfo.gps_timestamp = %lld", pt.latitude, pt.altitude, pt.longitude, pt.process_method, jpegInfo.gps_timestamp);
	...
	{
		HAL_LOGD("CAMERA_EVT_CB_HAL2_ZSL_NEW_FRM");
		break;
	}
	default:
										...
										void* parm4)
{
	...
	switch (cb) {
	case CAMERA_RSP_CB_SUCCESS:
		HAL_LOGD("camera cb: autofocus has started.");
		break;

	case CAMERA_EXIT_CB_DONE:
		HAL_LOGD("camera cb: autofocus succeeded.");
	{
	...
	case CAMERA_EVT_CB_FOCUS_MOVE:
		HAL_LOGD("camera cb: autofocus focus moving  %d autofocus=%d", parm4, mIsAutoFocus);
		if (!mIsAutoFocus) {
			if (parm4) {
					...
					void* parm4)
{
	...
	HAL_LOGD("E");
	HAL_LOGD("debug cb = %d func = %d parm4 = 0x%x", cb, func, (cmr_uint)parm4);
	switch(func) {
	...
	case ANDROID_CONTROL_AE_LOCK:
		{
			...
			//SET_PARM(mCameraHandle, CAMERA_PARAM_ISP_AE_LOCK_UNLOCK, ae_lock);
			if(ae_lock == 1) {
				controlInfo.ae_state = ANDROID_CONTROL_AE_STATE_LOCKED;
				mSetting->setCONTROLTag(controlInfo);
			} else {
	...
	case ANDROID_SPRD_ZSL_ENABLED:
		{
			...
			if(sprddefInfo.sprd_zsl_enabled == 0 && mRecordingMode == false) {
				mSprdZslEnabled = false;
				SET_PARM(mCameraHandle, CAMERA_PARAM_SPRD_ZSL_ENABLED, 0);
			}
		}
		break;
#endif
...
int SprdCamera3OEMIf::setCapturePara(camera_capture_mode_t cap_mode, uint32_t frame_number)
{
	...
	switch(cap_mode)
	{
		...
		case CAMERA_CAPTURE_MODE_ZSL_SNAPSHOT:
			...
			mZslPreviewMode = false;
			break;
		case CAMERA_CAPTURE_MODE_VIDEO_SNAPSHOT://fix bug 537051
...
int SprdCamera3OEMIf::Callback_Free(enum camera_mem_cb_type type, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_u32 sum, void* private_data)
{
	...
	if (CAMERA_PREVIEW == type) {
		...
		CAMERA_SNAPSHOT_ZSL_RESERVED_MEM == type || CAMERA_ISP_LSC == type || CAMERA_ISP_BINGING4AWB == type) {
...
int SprdCamera3OEMIf::PushZslbuff(buffer_handle_t * buff_handle)
{
	...
	if(channel &&
		...
		if(stream) {
			...
			HAL_LOGD("addr_phy = 0x%lx, addr_vir = 0x%lx, zsl_private = 0x%lx ret = %d",addr_phy, addr_vir, zsl_private, ret);
			if(ret == NO_ERROR) {
				if(addr_vir != NULL)
					camera_set_zsl_buffer(mCameraHandle, addr_phy, addr_vir, zsl_private);
				else
...
void SprdCamera3OEMIf::receiveZslFrame(struct camera_frame_type *frame)
{
	...
	camera_zsl_snapshot_need_pause(mCameraHandle, &need_pause);
	if (PREVIEW_ZSL_FRAME == frame->type) {
		HAL_LOGD("type = %d, mZslShotPushFlag %d", frame->type, mZslShotPushFlag);
		if (mZslShotPushFlag) {
			HAL_LOGD("getZSLQueueFrameNum %d", getZSLQueueFrameNum());
			ret = releaseZslBuffer(frame);
			if (ret != 0) {
				HAL_LOGE("failed");
			}

			ret = getZSLSnapshotFrame(&mem_info);
			if (0 == ret) {
				HAL_LOGD("mem_info.addr_phy=0x%lx, mem_info.addr_vir=0x%lx", mem_info.addr_phy, mem_info.addr_vir);
				camera_set_zsl_snapshot_buffer(mCameraHandle, (cmr_uint)mem_info.addr_phy, (cmr_uint)mem_info.addr_vir);
				mZslShotPushFlag = 0;
			}
		} else {
			if (SPRD_INTERNAL_RAW_REQUESTED == getCaptureState()) {
				...
				if (mZslMaxFrameNum < getZSLQueueFrameNum()) {
					ret = getZslBuffer(&mem_info);
					if (0 == ret) {
						camera_set_zsl_buffer(mCameraHandle, (cmr_uint)mem_info.addr_phy, (cmr_uint)mem_info.addr_vir, (cmr_uint)mem_info.fd);
					}
				}
			}
		}
	...
	if (1 == obj->mZslShotPushFlag && 1 == obj->mZslChannelStatus) {
		...
		if (SPRD_INTERNAL_RAW_REQUESTED == obj->getCaptureState()) {
			if (obj->mZslMaxFrameNum < obj->getZSLQueueFrameNum() && 1 == obj->mZslChannelStatus) {
				...
				if(0 == ret) {
					camera_set_zsl_buffer(obj->mCameraHandle, (cmr_uint)mem_info.addr_phy, (cmr_uint)mem_info.addr_vir, (cmr_uint)mem_info.fd);
				}
			}
		}
	}
