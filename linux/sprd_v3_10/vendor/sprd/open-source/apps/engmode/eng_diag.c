...
static int eng_diag_read_publickey(char *buf,int len,char *rsp, int rsplen)
{
    ...
    *((char*)(rsp_ptr + sizeof(MSG_HEAD_T))) = 0x00;
    ...
    memset(publickeypath, 0 , sizeof(publickeypath));
...
static int eng_is_supplicant_running(void)
{
    ...
    {
        ENG_LOG("%s:Supplicant not running, cannot connect\n",__FUNCTION__);
        return 0;
    }
    else
    {
        ENG_LOG("%s:Supplicant is running\n",__FUNCTION__);
        return 1;
    }
}
...
static int eng_open_wifi_switch()
{
    ...
    ENG_LOG("%s:wifi module name [%s]\n",__FUNCTION__,WIFI_DRIVER_MODULE_NAME);
#endif
    ...
    if( wifi_load_driver() != 0 ) {
        ENG_LOG("%s:wifi_load_driver fail!\n",__FUNCTION__);
        return 0;
    }

    if(wifi_start_supplicant(0) != 0 ) {
        wifi_unload_driver();
        ENG_LOG("%s:wifi_start_supplicant fail!\n",__FUNCTION__);
        return 0;
    }
    ...
    while( sup_chk_num-- ) {
        ...
        if( eng_is_supplicant_running() != 1 ) {
            continue;
        } else {
            sup_chk_ret = 0;
            break;
    ...
    if( 0 != sup_chk_ret ) {
        wifi_stop_supplicant(0);
        wifi_unload_driver();
        ENG_LOG("%s:Supplicant is not ready!\n",__FUNCTION__);
        return 0;
    }
...
static int parse_config(void)
{
	...
	fclose(fp);
	if(count < 64) {
		ENG_LOG("mmitest parse slog.conf failed");
	}
