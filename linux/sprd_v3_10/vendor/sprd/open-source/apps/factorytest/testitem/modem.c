...
void* modem_init_func(void *arg)
{
	...
	if( 1 == mode_type)
	    property_get(PROP_MODEM_LTE_COUNT, modem_prop, "");
	else
	    property_get(PROP_MODEM_W_COUNT, modem_prop, "");
	s_modem_count = atoi(modem_prop);
	LOGD("mmitest get %s = %d", PROP_MODEM_W_COUNT, s_modem_count);
	if(s_modem_count > 0 && s_modem_count <= MAX_MODEM_COUNT) {
		for(i = 0; i < s_modem_count; i++) {
			while(1) {
			      if(1 == mode_type)
				    modem_fd[i] = open(modem_lte_port[i], O_RDWR);
			      else
				    modem_fd[i] = open(modem_w_port[i], O_RDWR);
				LOGD("open mode_type = %d,fd[%d] = %d",mode_type, i,modem_fd[i]);
				if(modem_fd[i] > 0) {
					break;
				}
				usleep(1000);
	...
	if(0 == mode_type){
	    for(i = 0; i < s_modem_count; i++) {
            if(modem_send_at(modem_fd[i], "AT+SFUN=2", NULL, 0, 20) < 0){
                return NULL;
            }
	    }
	}
