...
int test_cali_info(void)
{
	...
	int i,tp_row;
	int row = 2;
	char tmp[64][32],gsm_cali[64][32],wcdma_cali[64][32], lte_cali[64][32];
	char* pcur,*pos1,*pos2;
	...
	while(len > 0) {
		...
		if(pos2 == NULL) {
			...
			tmp[pos2-pos1][wcdma_num] = '\0';
			len -= (pos2 - pcur);
			pcur = pos2;
		}
		testlen=str_replace(wcdma_cali[wcdma_num],tmp[wcdma_num],"BAND","WCDMA BAND");
		LOGD("mmitest test=%s,wcdma_num = %d",wcdma_cali[wcdma_num],wcdma_num);
		wcdma_num++;
		//row = ui_show_text(row, 0, tmp2);
	}
start:
	do{
        if( 0 == change%3){
	      ...
	      if(has_show == 0)
	          remain_len = gsm_num;
		LOGD("mmitest gsm_cali[i+has_show]=%s",gsm_cali[i+has_show]);
