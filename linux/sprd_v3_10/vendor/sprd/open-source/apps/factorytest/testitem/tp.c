...
//GST_guojiangping modify 20150625 end
int tp_flag = 0;
...
//		LOGD("%s: x=%d; y=%d",__func__, eng_tsinfo.touch_point.x,eng_tsinfo.touch_point.y);
		x_pressed=0;
		y_pressed=0;
		...
		switch(event->code)
		{
			...
			case ABS_MT_POSITION_Y:
				...
				if(0) {
					...
					if( abs(last_pos.x - cur_pos.x) > 20 ||abs( last_pos.y - cur_pos.y) > 20){
...
int GetTpVendorInfo(void)
{
	...
	if(fd < 0){
		LOGD("open tp hw_info failed!!");
		return 0;
	}
	ret = read(fd,tp,sizeof(tp));
	close(fd);
	if(ret < 0){
		LOGD("Get TP HWinfo failed!!");
		return 0;
	}
...
static void* tp_thread(void *par)
{
	...
	//GST_guojiangping modify 20150625 begin
	ret = GetTpVendorInfo();
	if(tp_dev_info[ret].vendor == NULL){
		LOGD("get TP vendor failed!!\n");
		return NULL;
	}
	//GST_guojiangping modify 20150625 end
	int fd = -1;
	...
	if(fd < 0) {
		LOGD("mmitest open %s fail",SPRD_TS_INPUT_DEV);
#else
	fd = find_input_dev(O_RDONLY, tp_dev_info[ret].dev_name);
	if(fd < 0) {
		LOGD("mmitest open %s fail",tp_dev_info[ret].dev_name);
#endif
