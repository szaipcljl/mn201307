...
unsigned cwidth=0,cheight=0,ewidth=0,eheight=0;
int page_num = 0;
int page_shown = 0;
...
static void draw_screen_locked(void)
{
    ...
    if (show_text) {
		...
		menu_sel_tmp=menu_sel-page_num*page_shown;
        else
...
int ui_show_menu(char* title, char** items, int is_root, int sel,int itemnum)
{
    ...
    LOGD("mmitest uishow menu_change=%d",menu_change);
show:
    ...
    while (chosen_item < 0) {
        ...
        if (action < 0) {
            switch (action) {
			    ...
			    //has more than one page
                        ...
                        if(itemnum >= page_num){
                            ...
                            if((menu_change!=0&&menu_change!=menu_change_last) || ( 2 == menu_change && 0 == (selected%page_num))){
                                ui_start_menu(title, items, selectedtmp,itemnum);
                ...
                case GO_HOME:
                        LOGD("mmitest GO_HOME page_shown=%d",page_shown);
                        if(is_root){
                            chosen_item = -1;
                            menu_change = 0;
                            if(page_shown){
...
{
	int key = -1;
	if(left != NULL || right != NULL || center != NULL) {//centor != NULL ||
		ui_show_button(left, center,right);// centor,
		gr_flip();
	}
	ui_clear_key_queue();
	...
	for(;;) {
		...
		if(1==usbin_state){
			if((KEY_POWER!=key) && (KEY_VIR_FAIL!=key))
				key=-1;
		}
		if(((1==tp_flag)&&(((KEY_VIR_PASS==key) || (KEY_VIR_FAIL==key) || (KEY_VIR_NEXT_PAGE==key))))
		|| (((NULL==left) && (KEY_VIR_PASS==key)) || ((NULL==center) && (KEY_VIR_NEXT_PAGE==key)) || ((NULL==right) && (KEY_VIR_FAIL==key)))
