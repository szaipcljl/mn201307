...
static int vbr_write(exfat_boot_sector * bs, __u64 start)
{
	...
	__u32 checksum;
	__u32* sector = memalign(ARCH_DMA_MINALIGN, sec_size);
	size_t i;
...
static __u32 fat_alignment(void)
{
	return (__u32) 128 * get_sector_size();
}

static __u32 fat_size(void)
{
	return get_volume_size() / get_cluster_size() * sizeof(__u32);
}


static int fat_write(exfat_boot_sector * bs, __u64 start)
{
	__u32 c = 0;
	__u32  s, end, idx;
	__u32 sec_size = 1 << get_sector_bits();
...
{
	return (__u32) 1024 * get_sector_size();
}
...
static int cbm_write(exfat_boot_sector * bs, __u64 start)
{
	...
	__u8 * sector = memalign(ARCH_DMA_MINALIGN, sec_size);
...
static int init_objects()
{
	...
	fat.get_alignment = fat_alignment;
	fat.get_size = fat_size;
	fat.write = fat_write;
...
static int erase(exfat_boot_sector * bs)
{
	...
	__u64 position = 0;
	__u32  block_size = 1024 * sec_size;
	void* block = memalign(ARCH_DMA_MINALIGN, block_size);
		...
		__u64 first_sector)
{
	...
	if ((param.spc_bits + sector_bits) > 25) {
		debugf("Cluster size is too large: %d. It should not be larger than 32MB.", 1<<(param.spc_bits + sector_bits));
		param.spc_bits = 6;
	}
