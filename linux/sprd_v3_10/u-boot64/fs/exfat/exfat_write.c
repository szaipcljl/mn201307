...
static  exfat_dentry *empty_dentptr;
...
static int get_upcase_table(exfsdata *mydata, __u32 start_cl, __u32 length)
{
	...
	mydata->uptab_buf = memalign(ARCH_DMA_MINALIGN, rd_sec*mydata->sect_size);
...
update_dir_entry(exfsdata *mydata,  const char *filename, exfat_fnext_entry **fname_dent, exfat_dentry *entry, __u32 start_cluster,  __u64  written_size, int cont)
{
	exfat_file_entry *prim_entry = (exfat_file_entry *)entry;
	exfat_strext_entry * strext_entry = prim_entry->str_dent;
	timestamp_t  tm, *default_tm=&tm;
	__u32  creat_time;
	__u32  mod_time;
	__u32  access_time;
	__u8    creat_ms_inc;
	...
	default_tm->year = (2015 - 1980);
	default_tm->mon = 5;
	default_tm->day = 9;
	default_tm->hour = 11;
	default_tm->min = 12;
	default_tm->sec  = 13;
	creat_time = (default_tm->year << 9) | (default_tm->mon << 5) | default_tm->day;
	creat_time <<= 16;
	creat_time |=  (default_tm->hour <<11) | (default_tm->min << 5) |  (default_tm->sec/2) ;
	mod_time = access_time = creat_time;
	creat_ms_inc = mod_ms_inc = 0;
	creat_utc_off = mod_utc_off = access_utc_off = 0;
	prim_entry->mod_time  = cpu_to_le32(mod_time);
	prim_entry->access_time = cpu_to_le32(access_time);
	prim_entry->mod_ms_inc  = mod_ms_inc;
	prim_entry->mod_utc_off  = mod_utc_off;
	prim_entry->access_utc_off = access_utc_off;

	/* Filename checksum*/
	if (1){
		__u16 uni_name[FNAME_MAXLEN_BYTES];
		__u16 upcase_name[FNAME_MAXLEN_BYTES];
		__u16  val, checksum = 0;
		__u8 * ptr;
		int  i,  str_len;
		str_len = strlen(filename);
		for (i=0; i<str_len; i++) {
			uni_name[i] = (__u16)filename[i];
			upcase_name[i] = mydata->uptab_buf[uni_name[i]];
		}
		...
		strext_entry->name_hash = cpu_to_le16(checksum);
	}

	/* calculate primary checksum  */
	if (1) {
		__u8 *ptr = (__u8 *)(prim_entry);
		__u16  val, checksum = 0;
		int i, j, n;
		n = 32;
		for (i=0; i<n; i++) {
		...
		ptr = (__u8 *)(strext_entry);
		for (i=0; i<n; i++) {
			val = (__u16)ptr[i];
			checksum = ((checksum&1) ? 0x8000 : 0) + (checksum >>1) + val;
		}

		for (j=0; j<(prim_entry->secondary_cnt-1); j++) {
			ptr = (__u8 *)(fname_dent[j]);
			for (i=0; i<n; i++) {
				val = (__u16)ptr[i];
				checksum = ((checksum&1) ? 0x8000 : 0) + (checksum >>1) + val;
			}
		}
		prim_entry->checksum = cpu_to_le16(checksum);
	}
	return prim_entry;
}
...
static exfat_file_entry *
set_dir_entry(exfsdata *mydata, exfat_buf_array *bfs,  exfat_dentry *entry, const char * filename,
				__u32 start_cluster,   __u64  written_size, int cont)
{
	exfat_dentry  *tmp_entry,  *dentptr = entry;
	exfat_file_entry *prim_entry = (exfat_file_entry *)entry;
	...
	__u8 *bufend = NULL;
	__u8 *ptr,  *ptr_uni;
	__u32 next_sect, needed_clust, entry_num;
	__u32 gotclust, newclust=0, prevclust=0, start_sec;
	...
	tmp_buf = memalign(ARCH_DMA_MINALIGN, mydata->sect_size);
	if (tmp_buf == NULL) {
	...
	if (entry==NULL) {
		...
		clust_start_sec = mydata->data_begin +  bfs->bf1.cluster * mydata->clust_size;
		rem_bytes = cl_size - (bfs->bf1.start_sec-clust_start_sec)*mydata->sect_size;
		rem_bytes -= ((__u8 *)entry - bfs->bf1.pbuf);
	...
	if (needed_clust) {
		gotclust = get_available_clusters(mydata, &newclust, 1, 1);
		if (gotclust != 1) {
			debugf("Error: allocate new cluster for root directory failed. Got %d, start %d\n", gotclust, newclust);
			free(tmp_buf);
			return NULL;
		}
		set_exfatent_value(mydata, bfs->bf1.cluster,  newclust);
		set_exfatent_value(mydata, newclust,  0xffffffff);

		start_sec =  mydata->data_begin + newclust * mydata->clust_size;
		for (i=0; i<(int)mydata->clust_size; i++) {
			if (exfat_disk_write(start_sec,  1, tmp_buf) < 0) {
				free(tmp_buf);
		...
		if (prim_entry==NULL) {
			...
			bfs->bf1.cluster = newclust;
			bfs->bf1.sectors = 1;
			bfs->bf1.start_sec = mydata->data_begin + newclust * mydata->clust_size;
			memset(bfs->bf1.pbuf, 0, mydata->sect_size);
			prim_entry = (exfat_file_entry *)bfs->bf1.pbuf;
		}
	}
	...
	creat_ms_inc = mod_ms_inc = 0;
	creat_utc_off = mod_utc_off = access_utc_off = 0;
	attr = EXFAT_ATTR_FILE;
	prim_entry->entry_type = EXFAT_DTYPE_USED | EXFAT_DTYPE_FILE;
	prim_entry->secondary_cnt = (__u8)(entry_num&0xff);
	prim_entry->attr = cpu_to_le16(attr);
	prim_entry->creat_time = cpu_to_le32(creat_time);
	prim_entry->mod_time  = cpu_to_le32(mod_time);
	prim_entry->access_time = cpu_to_le32(access_time);
	prim_entry->creat_ms_inc = creat_ms_inc;
	prim_entry->mod_ms_inc  = mod_ms_inc;
	prim_entry->creat_utc_off = creat_utc_off;
	prim_entry->mod_utc_off  = mod_utc_off;
	prim_entry->access_utc_off = access_utc_off;

	tmp_entry = (exfat_dentry *)tmp_buf;
	*tmp_entry = *(exfat_dentry *)prim_entry;
	...
	while (entry_num-- >0) {
		dentptr++;
		tmp_entry++;
		bufend = bfs->bf1.pbuf + bfs->bf1.sectors * mydata->sect_size;
		if  ((__u8 *)dentptr >=bufend) {
			...
			if (needed_clust) {
				...
				bfs->bf1.cluster = bfs->bf0.cluster;
				bfs->bf1.start_sec = bfs->bf0.start_sec + bfs->bf0.sectors;
			}
			bfs->bf1.sectors = 1;
			memset(bfs->bf1.pbuf, 0, mydata->sect_size);
		...
		if (strext_entry==NULL) {
			...
			fname_entry = (exfat_fnext_entry *)dentptr;
			...
			*tmp_entry = *(exfat_dentry *)fname_entry;
		}
	}
	...
	tmp_entry = (exfat_dentry *)tmp_buf;
	tmp_entry++;
	*tmp_entry = *(exfat_dentry *)strext_entry;
	...
	if (1) {
		...
		__u16  val, checksum = 0;
		int i, n;
		n = 32*(1+prim_entry->secondary_cnt);
		for (i=0; i<n; i++) {
			if (i==2 || i==3)
				continue;
			val = (__u16)ptr[i];
			checksum = ((checksum&1) ? 0x8000 : 0) + (checksum >>1) + (val&0xff);
		}
		prim_entry->checksum = cpu_to_le16(checksum);
	}
	free(tmp_buf);
...
static  exfat_file_entry  *find_dir_entry(exfsdata *mydata,  exfat_buf_array *bfs , __u32 startsect,  char *filename, exfat_fnext_entry **fname_dent)
{
	...
	int  i, j,  fn_len, idx, fname_dent_idx, strend;
		...
		for (i = 0; i < (bfs->bf1.sectors * mydata->sect_size /sizeof(exfat_dentry));  i++, dentptr++) {
			...
			if (pending_strext) {
				...
				if ((dentptr->entry_type&EXFAT_DTYPE_USED) &&
					...
					pending_fnext--;
					fname_dent[fname_dent_idx++] = dentptr;
					for (j=0; j < 30; j+=2) {
					...
					if (pending_fnext==0 || strend==1) {
						...
						if (!strcmp(filename, l_name)) {
							debugf("Existing file %s is found. It will be overwritten.\n",  filename);
							ret_dent = prim_entry;
			...
			else if  ((dentptr->entry_type & EXFAT_DTYPE_MASK)== EXFAT_DTYPE_LABLE) {
				// skip it.
			}
			...
			else if ((dentptr->entry_type&EXFAT_DTYPE_USED)==0)  {
				// skip it.
			}
			else {
...
unsigned long  write_file_contents(exfsdata *mydata, __u32 *start_cluster,  __u8 *buffer, unsigned long filesize, int *cont)
{
	...
	*cont=0;
	...
	while (filesize > 0) {
		...
		if (actsize > 0) {
			if (write_clusters(mydata, curclust, buffer, actsize) != 0) {
	...
	unsigned long size)
{
	...
	mydata->fatbuf_idx = 0;
	mydata->fatbuf = memalign(ARCH_DMA_MINALIGN, mydata->sect_size);
	...
	mydata->bitmap_idx = 0;
	mydata->bitmap = memalign(ARCH_DMA_MINALIGN, mydata->sect_size);
	...
	/* if cl_size==512,  too long name length will cause one dir entry cross 3 clusters, which we can't process. */
	if(cl_size < 1024 && name_len > 210) {
	...
	retdent = find_dir_entry(mydata, &buf_array,  startsect,  l_filename, &fname_dent);
	...
	mydata->bitmap_fatbuf = memalign(ARCH_DMA_MINALIGN, mydata->bitmap_fatbuf_size * mydata->sect_size);
	...
	if (retdent != NULL) {
		// Clear previous content of file (fat table & bitmap)
	...
	int cont = 0;
	start_cluster = 0;
	written_size = write_file_contents(mydata, &start_cluster, buffer, size, &cont);
	if (written_size == 0) {
	...
	if (retdent==NULL) {
		//  Create a new file.
		retdent = set_dir_entry(mydata, &buf_array, empty_dentptr, filename, start_cluster, size64, cont);
		if (retdent==NULL) {
		...
		update_dir_entry(mydata, filename, &fname_dent, retdent, start_cluster, size64, cont);
	}

	/* Write directory table to device */
	if (buf_array.bf0.sectors> 0) {
		if (exfat_disk_write(buf_array.bf0.start_sec,  buf_array.bf0.sectors, buf_array.bf0.pbuf) < 0) {
			debugf("Error writing data\n");
			goto exit;
		}
	}
	if (buf_array.bf1.sectors > 0) {
		if (exfat_disk_write(buf_array.bf1.start_sec,  buf_array.bf1.sectors, buf_array.bf1.pbuf) < 0) {
			debugf("Error writing data\n");
			goto exit;
		}
	}
...
exit:
	...
	if (mydata->bitmap)
		free(mydata->bitmap);
	if (mydata->uptab_buf)
		free(mydata->uptab_buf);
	if(mydata->bitmap_fatbuf)
		free(mydata->bitmap_fatbuf);
...
{
	debugf("writing %s\n", filename);
	return do_exfat_write(filename, buffer, maxsize);
}
