...
for(i=0;i<512;i++)
{
	...
	Emmc_Write(0, 0xb000,1,testbuf_write);
	Emmc_Read(0, 0xb000,1,testbuf_read);
	
	for(i=0;i<128;i++)
	{
		printf("testbuf_read = 0x%x\n",testbuf_read[i]);
	}
	...
	Emmc_Read(0, 0,1,testbuf_read);	

	for(i=0;i<512;i++)
	{
		printf("block0 of emmc = 0x%x\n",testbuf_read[i]);
	}
#endif
#if 1
		for(j=4;j<5;j++)
	{
		...
		if (!Emmc_Write(0, j,1,testbuf_write))
			printf("Write emmc block 0x%x error!!\n", j);
		if (!Emmc_Read(0, j,1,testbuf_read))
			printf("Read emmc block 0x%x error!!\n", j);
		printf("testbuf_read = 0x%d,0x%d,0x%d,0x%d\n",testbuf_read[12],testbuf_read[25],testbuf_read[122],testbuf_read[125]);	
	}
		sdio_dump(EMMC);
...
int sprd_host_init(int sdio_type)
{
	struct mmc *mmc;
	uint32_t ret,i;
	SDIO_BASE_INFO* sprd_host_info;
	sprd_host_info  =  get_sdcontrol_info(sdio_type);
	LDO_TurnOnLDO(sprd_host_info->ldo_core);
	LDO_TurnOnLDO(sprd_host_info->ldo_io);
	#ifndef CONFIG_FPGA
	CHIP_REG_SET(sprd_host_info->baseclk_reg,sprd_host_info->baseclk_mask);
	#endif
	debugf("sprd_host_info->ldo_core:%d,sprd_host_info->ldo_io:%d\n",sprd_host_info->ldo_core,sprd_host_info->ldo_io);
	udelay(1000);
	CHIP_REG_OR(sprd_host_info->ahb_enable_reg,sprd_host_info->ahb_enable_bit);
	CHIP_REG_OR(sprd_host_info->ahb_reset_reg,sprd_host_info->ahb_reset_bit);
	CHIP_REG_AND(sprd_host_info->ahb_reset_reg,~(sprd_host_info->ahb_reset_bit));
...
int Emmc_Write(char  part_num, uint32_t startBlock,uint32_t num,uint8_t* buf)
{
	...
	ret = mmc_bwrite(EMMC,startBlock,num,buf);
	if(part_num){
		mmc->part_num = 0;
		mmc_switch_part(EMMC, 0);
	 }
...
int Emmc_Write_Backstage(char  part_num, uint32_t startBlock,uint32_t num,uint8_t* buf)
{
	struct mmc *mmc = find_mmc_device(EMMC);
	int ret = 0;
	if (!mmc)
	...
	ret = mmc_bwrite_backstage(EMMC,startBlock,num,buf);

	return ret;
}

int Emmc_Query_Backstage(char  part_num, uint32_t num,uint8_t* buf)
{
	struct mmc *mmc = find_mmc_device(EMMC);
	int ret = 0;
	if (!mmc)
...
int Emmc_Erase(char  part_num, uint32_t startBlock,uint32_t num)
{
	struct mmc *mmc = find_mmc_device(EMMC);
	int ret = 0;
	if (!mmc)
	...
	ret = mmc_berase(EMMC,startBlock,num);
	if(part_num){
		mmc->part_num = 0;
		mmc_switch_part(EMMC, 0);
	 }
...
int Emmc_Read(char  part_num, uint32_t startBlock,uint32_t num,uint8_t* buf)
{
	struct mmc *mmc = find_mmc_device(EMMC);
	int ret = 0;
	if (!mmc)
	...
	mmc->part_num = part_num;
	debugf("Emmc_Read partnum=%d,startBlock=%x,num=%d,buf=%p\n",part_num,startBlock,num,buf);
...
uint64_t Emmc_GetCapacity(char  part_num)
{
	struct mmc *mmc = find_mmc_device(EMMC);
	if (!mmc)
		return 0;
