...
dsih_error_t mipi_dsih_open(dsih_ctrl_t * instance)
{
    ...
    mipi_dsih_hal_power(instance, 0);//tianci 1.21a modify
    //mipi_dsih_hal_power(instance, 0);//Jessica masked
    //mipi_dsih_hal_power(instance, 1);//Jessica masked
    ...
    //err = mipi_dsih_phy_hs2lp_config(instance, instance->max_hs_to_lp_cycles);//tianci 1.21a modify
    //err |=  mipi_dsih_phy_lp2hs_config(instance, instance->max_lp_to_hs_cycles);//tianci 1.21a modify

    mipi_dsih_hal_int_mask_0(instance, 0xffffff);//tianci 1.21a modify
    mipi_dsih_hal_int_mask_1(instance, 0xffffff);//tianci 1.21a modify
...
void mipi_dsih_video_mode(dsih_ctrl_t * instance, int en)
{
    ...
    if (instance->status == INITIALIZED)// tianci need change
    {
...
uint32_t mipi_dsih_dump_register_configuration(dsih_ctrl_t * instance, int all, register_config_t *config, uint16_t config_length)
{
    ...
    {
        /* dump all registers */
        for (current = R_DSI_HOST_VERSION; current <= R_DSI_HOST_PHY_TMR_LPCLK_CFG; count++, current += (R_DSI_HOST_PWR_UP - R_DSI_HOST_VERSION))
        {
