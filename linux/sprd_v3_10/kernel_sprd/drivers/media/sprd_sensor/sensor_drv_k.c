...
#if 1 //huafeizhou141014 add-s
#include <linux/kobject.h>

LOCAL uint16_t main_sensor_id=0,sub_sensor_id=0;
LOCAL char *main_sensor_name="unknown",*sub_sensor_name="unknown";
LOCAL char back_sensor_array[20];
LOCAL char front_sensor_array[20];

static ssize_t dc_information_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	//printk("dc_information_show:back_sensor_name=%s,,front_sensor_name=%s\n",main_sensor_name,sub_sensor_name);
     	//return sprintf(buf, "Back camera:\n%s , ID: 0x%4X\nFront camera:\n%s , ID: 0x%4X\n", sensor_info.back_sensor_name,sensor_info.back_sensor_id,sensor_info.front_sensor_name,sensor_info.front_sensor_id);
   	return sprintf(buf, "Back Camera:\n%s , ID: 0x%04X\nFront Camera:\n%s , ID: 0x%04X\n", main_sensor_name,main_sensor_id,sub_sensor_name,sub_sensor_id);
   	//return sprintf(buf, "Back camera: 0x%4X\nFront camera: 0x%4X\n",main_sensor_id,sub_sensor_id);
   	//return sprintf(buf, "Back camera:%s \nFront camera:%s \n", main_sensor_name,sub_sensor_name);
}

static struct kobj_attribute dc_information = {
    .attr = {
        .name = "dc_information",
        .mode = S_IRUGO,
    },
    .show  = &dc_information_show,
    .store = NULL,
};
...
static void dc_info_init(void)
{
    int ret = 0;
    struct kobject *properties_kobj;

    printk("[sensor_drv_k] %s\n",__func__);

    properties_kobj = kobject_create_and_add("board_properties3", NULL);
    if (properties_kobj)
        ret = sysfs_create_group(properties_kobj,
                     &properties_attr_group);
    if (!properties_kobj || ret)
        printk("failed to create board_properties3\n");
}
#endif
...
#endif
int sensor_k_set_afenable_level(uint32_t *fd_handle,uint8_t power_level)
{
	...
	get_af_gpio_id(p_mod->of_node, &fd->gpio_tab.af_enable,fd->sensor_id);
    SENSOR_PRINT_HIGH("SENSOR: af_enable %d\n", power_level);

	if (0 == power_level) {
		gpio_direction_output(fd->gpio_tab.af_enable, 0);
	} else {
		gpio_direction_output(fd->gpio_tab.af_enable, 1);
	}
...
LOCAL long sensor_k_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SENSOR_IO_PD:
		...
		break;
	case SENSOR_IO_AF_PD:
		{	 
			uint8_t power_level;
			ret = copy_from_user(&power_level, (uint8_t *) arg, sizeof(uint8_t));

			if (0 == ret) 
				ret = sensor_k_set_afenable_level((uint32_t *)p_file,power_level);
		}	 
		break;
	...
	case SENSOR_IO_IF_CFG:
		...
		break;
	#if 1 //huafeizhou141014 add-s
	case SENSOR_IO_GET_ID:
		{
			SENSOR_HW_INFO_T sensor_info;

			ret = copy_from_user(&sensor_info, (SENSOR_HW_INFO_T *) arg, sizeof(SENSOR_HW_INFO_T));
			printk("1sensor_k_ioctl:back_sensor_name=%s,back_sensor_id:0x%4x,front_sensor_name=%s,front_sensor_id:0x%4x\n",sensor_info.back_sensor_name,sensor_info.back_sensor_id,sensor_info.front_sensor_name,sensor_info.front_sensor_id);

			if (0 == ret) {
				memset(back_sensor_array,0,sizeof(back_sensor_array));
				strcpy(back_sensor_array,sensor_info.back_sensor_name);
				...
				sub_sensor_id=sensor_info.front_sensor_id;
				printk("3sensor_k_ioctl:front_sensor_array=%s\n",front_sensor_array);
			}
			printk("4sensor_k_ioctl:main_sensor_name=%s,sub_sensor_name=%s\n",main_sensor_name,sub_sensor_name);
