...
#include <linux/i2c/epl259x_v3_0.h>
...
#define ELAN_INT_PIN 239 //129    /*Interrupt pin setting*/
...
#endif
#if !SPREAD //huafeizhou151217 mod /*it must use als input dev*/
...
int epl_sensor_read_als(struct i2c_client *client)
{
	...
	#if 0 //huafeizhou151217 del  /*too much log cause adb tool abnormal*/
	LOG_INFO("als: ~~~~ ALS ~~~~~ \n");
...
int epl_sensor_read_ps(struct i2c_client *client)
{
	...
	#if 1 //huafeizhou151217 del  /*too much log cause adb tool abnormal*/
	LOG_INFO("ps: ~~~~ PS ~~~~~ \n");
...
static void epl_sensor_polling_work(struct work_struct *work)
{
   ...
   // printk("***epl_sensor_polling_work:enable_als=%d,polling_mode=%d\n",enable_als,epl_sensor.als.polling_mode);//huafeizhou151217 add for debug
    if(enable_als &&  epl_sensor.als.polling_mode == 1)
    {
       ...
       // printk("***epl_sensor_polling_work:report_lux=%d\n",report_lux); //huafeizhou151217 add for debug
        epl_sensor_report_lux(report_lux);
    }

    if(enable_ps && epl_sensor.ps.polling_mode == 1)
    {
        printk("***ps.polling_mode=%d\n",epl_sensor.ps.polling_mode); //huafeizhou151217 add for debug
#if !PS_DYN_K
        epl_sensor_read_ps(client);
#endif
