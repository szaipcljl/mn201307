...
#if 1 //add use keypad mode for key detect,default is gpio mode
...
void board_keypad_init(void)
{
	...
	REG_KPD_CTRL  = (1<<8)/*Col0  Enable*/|(3<<16)/*Row0-Row1 Enable*/; //huafeizhou151218 add
...
unsigned int check_key_boot(unsigned char key)  //检测按键启动进入对应模式
{
	if(KEY_VOLUMEUP == key)
