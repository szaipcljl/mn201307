...
cmr_int sns_dev_afenable_lvl(struct sensor_drv_context *sensor_cxt,
					cmr_u32 level)
{
	cmr_int ret = SENSOR_SUCCESS;
	SENSOR_DRV_CHECK_ZERO(sensor_cxt);
	...
	CMR_LOGV("fd 0x%lx", sensor_cxt->fd_sensor);
	ret = ioctl(sensor_cxt->fd_sensor, SENSOR_IO_AF_PD, &level);
	if (0 != ret) {
		CMR_LOGE("failed,  level = %d, ret=%ld ", level, ret);
		ret = -1;
	}
...
cmr_int Sensor_Set_AfEnableLevel(cmr_u32 plus_level)
{
    struct sensor_drv_context *sensor_cxt = (struct sensor_drv_context *)sensor_get_dev_cxt();
	if (-1 == sns_dev_afenable_lvl(sensor_cxt, (cmr_u32)plus_level))
		return SENSOR_FAIL;
...
#if 1////huafeizhou141015 add
LOCAL uint16_t back_sensor_id=0,front_sensor_id=0;
LOCAL char *back_sensor_name=PNULL,*front_sensor_name=PNULL;

LOCAL int _Sensor_Device_GetID(struct sensor_drv_context *sensor_cxt,SENSOR_HW_INFO_T_PTR sensor_info)
{
	int ret = SENSOR_SUCCESS;
	SENSOR_DRV_CHECK_ZERO(sensor_cxt);

	//SENSOR_REG_BITS_T reg;
	//SENSOR_REG_BITS_T_PTR reg2;

	//reg.reg_addr=back_id;
	//reg.reg_value=front_id;

	CMR_LOGI("_Sensor_Device_GetID:back_sensor_name=%s,back_sensor_id:0x%4x,front_sensor_name=%s,front_sensor_id:0x%4x\n",sensor_info->back_sensor_name,sensor_info->back_sensor_id,sensor_info->front_sensor_name,sensor_info->front_sensor_id);
	ret = ioctl(sensor_cxt->fd_sensor, SENSOR_IO_GET_ID, sensor_info);
	if (0 != ret)
	{
		CMR_LOGI("_Sensor_Device_GetID failed, ret=%d \n",ret);
		ret = -1;
	}

	return ret;
}
#endif//huafeizhou141015 add
...
#if 1 //huafeizhou141015 add-s
	{

		SENSOR_HW_INFO_T senror_info;

		CMR_LOGI("***sensor_init:sensor_identified=%d,is_register[0]=%d,is_register[1]=%d\n",sensor_cxt->sensor_identified,sensor_cxt->sensor_register_info.is_register[0],sensor_cxt->sensor_register_info.is_register[1]);
		if(sensor_cxt->sensor_list_ptr[0]==PNULL)
			CMR_LOGI("***sensor_init:sensor_list_ptr[0]=PNULL\n");
		if(sensor_cxt->sensor_list_ptr[1]==PNULL)
			CMR_LOGI("***sensor_init:sensor_list_ptr[1]=PNULL\n");
			
		if(sensor_cxt->sensor_identified == SCI_TRUE)
		{
			if((sensor_cxt->sensor_register_info.is_register[0]==1) && (sensor_cxt->sensor_list_ptr[0]!=PNULL))
			{
				back_sensor_name=sensor_cxt->sensor_list_ptr[0]->name;
				back_sensor_id=((sensor_cxt->sensor_list_ptr[0]->identify_code[0].reg_value)<<8) |(sensor_cxt->sensor_list_ptr[0]->identify_code[1].reg_value);
			}
			else
			{
				back_sensor_name="unknown";
				back_sensor_id=0;
			}

			if((sensor_cxt->sensor_register_info.is_register[1]==1) && (sensor_cxt->sensor_list_ptr[1]!=PNULL))
			{
				front_sensor_name=sensor_cxt->sensor_list_ptr[1]->name;
				front_sensor_id=((sensor_cxt->sensor_list_ptr[1]->identify_code[0].reg_value)<<8) |(sensor_cxt->sensor_list_ptr[1]->identify_code[1].reg_value);
			}
			else
			{
				front_sensor_name="unknown";
				front_sensor_id=0;			
			}
		}
		else
		...
		CMR_LOGI("***sensor_init:back_sensor_name=%s,back_sensor_id:0x%4x,front_sensor_name=%s,front_sensor_id:0x%4x\n",back_sensor_name,back_sensor_id,front_sensor_name,front_sensor_id);
		senror_info.back_sensor_name=back_sensor_name;
		senror_info.back_sensor_id=back_sensor_id;
		senror_info.front_sensor_name=front_sensor_name;
		senror_info.front_sensor_id=front_sensor_id;
		_Sensor_Device_GetID(sensor_cxt,&senror_info);
		CMR_LOGI("***sensor_init2:back_sensor_name=%s,back_sensor_id:0x%4x,front_sensor_name=%s,front_sensor_id:0x%4x\n",senror_info.back_sensor_name,senror_info.back_sensor_id,senror_info.front_sensor_name,senror_info.front_sensor_id);
	}
#endif//huafeizhou141015 add-e
