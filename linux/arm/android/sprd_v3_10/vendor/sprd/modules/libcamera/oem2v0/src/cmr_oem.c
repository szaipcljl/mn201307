...
static void camera_focus_evt_cb(enum af_cb_type cb, cmr_uint param, void *privdata);
...
ctrl_flash:
	...
	ret = cmr_setting_ioctl(setting_handle, CAMERA_PARAM_ISP_FLASH, &setting_param);
	if (ret) {
...
/*	case ISP_FLASH_AE_CALLBACK:
	...
	case ISP_AE_STAB_CALLBACK:
		ret = cmr_setting_isp_notice_done(cxt->setting_cxt.setting_handle, data);
		break;
...
*/
	...
	if (is_support_reload) {
		...
		ret = cmr_sensor_ioctl(cxt->sn_cxt.sensor_handle, cxt->camera_id, SENSOR_ACCESS_VAL, (cmr_uint)&val);
		if (ret || 0 == param_ptr.len) {
			CMR_LOGE("read otp data checksum failed %ld,%d", ret, param_ptr.len);
			goto EXIT;
		}
...
cmr_int camera_isp_init(cmr_handle  oem_handle)
{
	...
	if (ret) {
		CMR_LOGE("merge isp param %ld", ret);
		goto exit;
	}
...
cmr_int camera_isp_deinit(cmr_handle  oem_handle)
{
	...
	if(cxt->isp_anti_flicker_flag == 1) {
		if (cxt->hal_free) {
			cxt->hal_free(CAMERA_ISP_ANTI_FLICKER,&cxt->isp_anti_flicker_phys_addr,&cxt->isp_anti_flicker_virt_addr,
				ISP_ANTI_FLICKER_BUF_NUM, cxt->client_data);
		}
		cxt->isp_anti_flicker_flag = 0;
	}
#endif
...
sensor_deinit:
	...
	if(ret){
		CMR_LOGE("failed to camera_sensor_deinit %ld", ret);
	}
res_deinit:
...
cmr_int camera_raw_proc(cmr_handle oem_handle, cmr_handle caller_handle, struct raw_proc_param *param_ptr)
{
	...
	if (1 == param_ptr->slice_num) {
		...
		in_param.src_frame.format_pattern = param_ptr->src_frame.format_pattern;
...
cmr_int camera_isp_start_video(cmr_handle oem_handle, struct video_start_param *param_ptr)
{
	...
	CMR_LOGE("lsc_phys_addr 0x%x,lsc_virt_addr 0x%x",isp_param.lsc_phys_addr,
	...
	if (cxt->isp_anti_flicker_flag == 0) {
		isp_param.anti_flicker_buf_num = ISP_ANTI_FLICKER_BUF_NUM;
		isp_param.anti_flicker_buf_size = ISP_ANTI_FLICKER_BUF_SIZE;

		if (cxt->hal_malloc) {
			cxt->hal_malloc(CAMERA_ISP_ANTI_FLICKER, &isp_param.anti_flicker_buf_size, &isp_param.anti_flicker_buf_num,
					&cxt->isp_anti_flicker_phys_addr, &cxt->isp_anti_flicker_virt_addr, cxt->client_data);
			cxt->isp_anti_flicker_flag = 1;
		} else {
			ret = -CMR_CAMERA_NO_MEM;
			CMR_LOGE("failed to malloc isp anti-flicker buffer");
			goto exit;
		}

	}

	isp_param.anti_flicker_phys_addr = cxt->isp_anti_flicker_phys_addr;
	isp_param.anti_flicker_virt_addr = cxt->isp_anti_flicker_virt_addr;

	CMR_LOGE("anti_flicker_phys_addr 0x%x, anti_flicker_virt_addr 0x%x",isp_param.anti_flicker_phys_addr,
		isp_param.anti_flicker_virt_addr);
...
cmr_int camera_local_highflash_ae_measure (cmr_handle oem_handle)
{
	...
	if (CAMERA_ZSL_MODE != cxt->snp_cxt.snp_mode && (CAMERA_ISP_TUNING_MODE != snp_cxt->snp_mode)
		&& (CAMERA_ISP_SIMULATION_MODE != snp_cxt->snp_mode)) {
		setting_param.camera_id = cxt->camera_id;
...
cmr_int camera_local_start_snapshot(cmr_handle oem_handle, enum takepicture_mode mode, cmr_uint is_snapshot)
{
	...
	if (CAMERA_ISP_SIMULATION_MODE == mode) {
		...
		cmr_sensor_update_isparm_from_file(cxt->sn_cxt.sensor_handle, cxt->camera_id);
		isp_ioctl(isp_cxt->isp_handle, ISP_CTRL_DENOISE_PARAM_UPDATE, (void*)&nr_update_param);
		camera_isp_deinit(oem_handle);
...
#endif
			ret = isp_ioctl(isp_cxt->isp_handle, ISP_CTRL_TOOL_SET_SCENE_PARAM, (void*)&scene_param);
			if (ret) {
				CMR_LOGE("failed isp ioctl %ld", ret);
			}

			if ((scene_param.width > isp_cap_raw.size.width) || (scene_param.height > isp_cap_raw.size.height)) {
				ret = -CMR_CAMERA_INVALID_PARAM;
...
cmr_int camera_get_senor_mode_trim(cmr_handle oem_handle, struct img_rect *sn_trim)
{
	...
	if(sensor_mode >= SENSOR_MODE_MAX){
		//note:cmr_sensor_get_mode would not set parameter.so sensor_mode ==SENSOR_MODE_MAX
