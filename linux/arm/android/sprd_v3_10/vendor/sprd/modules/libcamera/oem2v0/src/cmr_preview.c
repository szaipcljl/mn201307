...
static cmr_int prev_set_zsl_buffer(struct prev_handle *handle, cmr_u32 camera_id, cmr_uint src_phy_addr, cmr_uint src_vir_addr, cmr_uint zsl_private);
...
cmr_int cmr_preview_set_zsl_buffer(cmr_handle preview_handle, cmr_u32 camera_id, cmr_uint src_phy_addr, cmr_uint src_vir_addr, cmr_uint zsl_private)
{
	...
	inter_param->param2 = (void*)src_phy_addr;
	inter_param->param3 = (void*)src_vir_addr;
	inter_param->param4 = (void*)zsl_private;
...
cmr_int prev_assist_thread_proc(struct cmr_msg *message, void *p_data)
{
	...
	case PREV_EVT_SET_ZSL_BUFFER:
		...
		src_phy_addr = (cmr_uint)inter_param->param2;
		src_vir_addr = (cmr_uint)inter_param->param3;
		zsl_private = (cmr_uint)inter_param->param4;

		ret = prev_set_zsl_buffer(handle, camera_id, src_phy_addr, src_vir_addr, zsl_private);
		break;
	default:
...
cmr_int prev_cb_start(struct prev_handle *handle, struct prev_cb_info *cb_info)
{
	...
	CMR_LOGV("cb_type %d, func_type %d, frame_data 0x%p",
		cb_data_info->cb_type,
		cb_data_info->func_type,
		cb_data_info->frame_data);
...
cmr_int prev_preview_frame_handle(struct prev_handle *handle, cmr_u32 camera_id, struct frm_info *data)
{
	...
	CMR_LOGD("got one frame, frame_id 0x%x, frame_real_id %d, channel_id %d yaddr 0x%x",
...
cmr_int prev_video_frame_handle(struct prev_handle *handle, cmr_u32 camera_id, struct frm_info *data)
{
	...
	/*skip frame if SW skip mode*/
	if (IMG_SKIP_SW == prev_cxt->skip_mode) {
		if (prev_cxt->video_frm_cnt <= prev_cxt->video_skip_num) {
...
cmr_int prev_recovery_post_proc(struct prev_handle *handle, cmr_u32 camera_id, enum recovery_mode mode)
{
	...
	if (IDLE == prev_cxt->prev_status && prev_cxt->prev_param.preview_eb && prev_cxt->prev_param.snapshot_eb) {
...
cmr_int prev_alloc_cap_buf(struct prev_handle *handle, cmr_u32 camera_id, cmr_u32 is_restart, struct buffer_cfg *buffer)
{
	...
	if (ZOOM_POST_PROCESS == zoom_post_proc || ZOOM_POST_PROCESS_WITH_TRIM == zoom_post_proc) {
		...
		channel_size = prev_cxt->actual_pic_size.width * prev_cxt->actual_pic_size.height;
	}
	...
	if (!is_restart) {
		...
		if ((IMG_DATA_TYPE_YUV420 == prev_cxt->cap_org_fmt || IMG_DATA_TYPE_YVU420 == prev_cxt->cap_org_fmt) && is_normal_cap) {
			...
			for (i = 0; i < cap_sum; i++) {
				...
				if ((0 == prev_cxt->cap_virt_addr_path_array[i]) || (0 == prev_cxt->cap_phys_addr_path_array[i])) {
	...
	if ((IMG_DATA_TYPE_YUV420 == prev_cxt->cap_org_fmt || IMG_DATA_TYPE_YVU420 == prev_cxt->cap_org_fmt) && is_normal_cap/* && i > 0*/) {
		...
		prev_cxt->cap_frm[i].addr_phy.addr_v = 0;
		prev_cxt->cap_frm[i].addr_vir.addr_y = prev_cxt->cap_virt_addr_path_array[i];
		prev_cxt->cap_frm[i].addr_vir.addr_u = prev_cxt->cap_frm[i].addr_vir.addr_y + buffer_size;
...
cmr_int prev_get_sensor_mode(struct prev_handle *handle, cmr_u32 camera_id)
{
	...
	if(1 == handle->prev_cxt[camera_id].prev_param.preview_eb){
		aligned_type = CAMERA_MEM_NO_ALIGNED;
	}
...
cmr_int prev_cap_ability(struct prev_handle *handle, cmr_u32 camera_id, struct img_size *cap_size, struct img_frm_cap *img_cap)
{
	...
	if (ZOOM_POST_PROCESS == prev_cxt->cap_zoom_mode) {
		...
		if (IMG_DATA_TYPE_RAW == prev_cxt->cap_org_fmt || ZOOM_POST_PROCESS == zoom_post_proc) {
			sn_trim_rect->start_x = img_cap->src_img_rect.start_x;
			sn_trim_rect->start_y = img_cap->src_img_rect.start_y;
			sn_trim_rect->width   = img_cap->src_img_rect.width;
			sn_trim_rect->height  = img_cap->src_img_rect.height;
				...
				struct snp_proc_param *cap_post_proc_param)
{
	...
	if (ZOOM_BY_CAP == prev_cxt->cap_zoom_mode || ZOOM_POST_PROCESS_WITH_TRIM == prev_cxt->cap_zoom_mode) {
		rect.start_x = 0;
		rect.start_y = 0;
...
cmr_int prev_set_preview_buffer(struct prev_handle *handle, cmr_u32 camera_id, cmr_uint src_phy_addr, cmr_uint src_vir_addr)
{
	...
	if (PREV_CHN_IDLE == prev_cxt->prev_channel_status) {
		CMR_LOGI("prev_channel_status idle");
		return ret;
	}
...
exit:
	...
	CMR_LOGD("done cnt %ld yaddr 0x%x", prev_cxt->prev_mem_valid_num, data->yaddr);
	return ret;
}
...
cmr_int prev_set_zsl_buffer(struct prev_handle *handle, cmr_u32 camera_id, cmr_uint src_phy_addr, cmr_uint src_vir_addr, cmr_uint zsl_private)
{
	cmr_int                     ret = CMR_CAMERA_SUCCESS;
	struct prev_context         *prev_cxt = NULL;
	cmr_int                     valid_num = 0;
...
exit:
	CMR_LOGD("out cnt %ld width %ld height %ld addr_y 0x%lx, addr_u 0x%lx, zsl_priv 0x%lx", prev_cxt->cap_zsl_mem_valid_num, width, height,
		prev_cxt->cap_zsl_frm[valid_num].addr_phy.addr_y, prev_cxt->cap_zsl_frm[valid_num].addr_phy.addr_u, zsl_private);
	return ret;
}
