...
static cmr_uint hdr_get_size()
{
#if defined(CONFIG_CAMERA_SUPPORT_13M)
        return  (20 << 20); // 13MEGA fullsize YUV420 : 13M*1.5
#elif defined(CONFIG_CAMERA_SUPPORT_8M)
        return  (13 << 20);// 8MEGA fullsize YUV420 : 8M*1.5
#elif defined(CONFIG_CAMERA_SUPPORT_7M)
        return  (11 << 20);// 7MEGA fullsize YUV420 : 7M*1.5
#elif defined(CONFIG_CAMERA_SUPPORT_6M)
        return  (10 << 20);// 6MEGA fullsize YUV420 : 6M*1.5
#elif defined(CONFIG_CAMERA_SUPPORT_5M)
        return  (8 << 20);// 5MEGA fullsize YUV420 : 5M*1.5
#elif defined(CONFIG_CAMERA_SUPPORT_4M)
        return  (7 << 20);// 4MEGA fullsize YUV420 : 4M*1.5
#elif defined(CONFIG_CAMERA_SUPPORT_3M)
        return  (5 << 20);// 3MEGA fullsize YUV420 : 3M*1.5
#elif defined(CONFIG_CAMERA_SUPPORT_2M)
        return  (3 << 20);// 2MEGA fullsize YUV420 : 2M*1.5
#else
        return  (13 << 20);
#endif
}
				...
				cmr_handle *class_handle)
{
	...
	size = hdr_get_size();//(cmr_uint)(in->frame_size.width * in->frame_size.height * 3 / 2);
...
static cmr_int hdr_save_frame(cmr_handle class_handle, struct ipm_frame_in *in)
{
	...
	if (hdr_handle->mem_size >= in->src_frame.buf_size) {
		...
		CMR_LOGW(" ERROR Not enough memory for saving source frame !!!HDR:mem size:0x%lx,data y_size:0x%lx.",hdr_handle->mem_size,y_size);
	}
