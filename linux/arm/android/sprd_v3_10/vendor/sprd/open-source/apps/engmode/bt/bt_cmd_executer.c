...
static unsigned char fm_enabled = 0;
static unsigned char fm_tune_status=0;
static unsigned char fm_rssi_status = 0;
...
{
char buf[40] ={0};
sprintf(buf,"fm enable callback status:%d",status);
BTENG_LOGD("btfmEnableCallback comming\n");
...
char buf[40] ={0};
fm_enabled=0;
sprintf(buf,"fm disable callback status:%d",status);

    if (sFmInterface) {
	BTENG_LOGD("FM_btInterface cleanup");	
	sFmInterface->cleanup();
    	}
    if (sBtInterface) {
	 BTENG_LOGD("FM_btInterface disable Radio");
...
BTENG_LOGD("btfmDisableCallback comming\n");
#if 0
    if (status == BT_STATUS_SUCCESS)
...
char buf[40] ={0};
fm_tune_status=1;
sprintf(buf,"fm tune callback status:%d,freq:%d",status,freq);
bteng_send_back_cmd_result(bteng_client_fd, buf, 1);

BTENG_LOGD("btfmtuneCallback comming\n");
#if 0
...
static void btfmGetRssiCallback(int status, int rssi)
{
char buf[40] ={0};
fm_rssi_status=1;
sprintf(buf,"fm getRSSI callback status:%d,RSSI:%d",status,rssi);
bteng_send_back_cmd_result(bteng_client_fd, buf, 1);
BTENG_LOGD("btfmGetRssiCallback comming\n");
}
...
static btfm_callbacks_t btfm_callbacks = {
    ...
    NULL,               // btfm_mute_callback
    NULL,             // btfm_search_callback
    NULL,     // btfm_search_complete_callback
    NULL,                           // btfm_af_jump_callback
    NULL,          // btfm_audio_mode_callback
    NULL,          // btfm_audio_path_callback
...
/* should add fm module init and btif_fm_enable here */
static void bteng_adapter_state_changed(bt_state_t state)
{
    ...
    char *p = NULL;
    int retVal;
    BTENG_LOGD("BTFM status have changed");
	...
	else if (state == BT_RADIO_ON)
	{
	fm_enabled = 1;
	BTENG_LOGD("fm_on_bteng_adapter_state_changed");
        sFmInterface = (btfm_interface_t*)sBtInterface->get_fm_interface();

        retVal = sFmInterface->init(&btfm_callbacks);
        retVal = sFmInterface->enable(96);
	}

	else if (state == BT_RADIO_OFF)
	{
	fm_enabled = 0;
	BTENG_LOGD("fm_off_bteng_adapter_state_changed");
	}	
...
static BT_ENG_ERROR_E bteng_fm_enable(void)
{
    bt_status_t status;
    BT_ENG_ERROR_E ret_val;

    BTENG_LOGD("ENABLE FM Radio");

    if (fm_enabled)
    {
        BTENG_LOGD("FM is already enabled");
        return BT_ENG_ENABLE_ERROR;
    }
    status = sBtInterface->enableRadio();

    bteng_check_return_status(status);
   BTENG_LOGD("enable radio status=%d\n",status);
    if(status == BT_STATUS_SUCCESS)
    {
        ret_val = BT_ENG_NONE_ERROR;
    }
...
static BT_ENG_ERROR_E bteng_fm_on(void)
{
	int err;
    char dut_mode = '1';

	BTENG_LOGD("bteng_fm_on\n");

    if(fm_enabled == 1)
    {
        BTENG_LOGD("%s, fm is already on\n", __FUNCTION__);
        bteng_adapter_state_changed(BT_RADIO_ON);
...
static BT_ENG_ERROR_E bteng_fm_off(void)
{

	int err;
	int counter = 0;

	BTENG_LOGD("bteng_fm_off\n");

	

    if(fm_enabled == 0)
    {
        BTENG_LOGD("%s, fm is already off\n", __FUNCTION__);
        bteng_adapter_state_changed(BT_RADIO_OFF);
    ...
    if (sFmInterface)
    	{
    	BTENG_LOGD("FM_sFmInterface disable");
        sFmInterface->disable();
    	}
    else
        return 0;
 BTENG_LOGD("fm_enable=%d\n",fm_enabled);
    while (counter++ < 3 &&(fm_enabled==0)) sleep(1);
#if 0	
...
static BT_ENG_ERROR_E bteng_fm_tune( int freq)
{
	int err;
//	int counter = 0;

	BTENG_LOGD("bteng_fm_tune freq=%d\n",freq);
    if (sFmInterface)
    	{
    	BTENG_LOGD("FM_sFmInterface tune");
        err=sFmInterface->tune(freq);
    	}
    else
        return 0;
    return err;
...
static BT_ENG_ERROR_E bteng_fm_get_rssi(void)
{

	int err;
	int counter = 0;

	BTENG_LOGD("bteng_fm_get_RSSI\n");
    if (sFmInterface)
    	{
    	BTENG_LOGD("FM_sFmInterface get_RSSI");
       err= sFmInterface->get_rssi();
    	}
    else
        return 0;
     return err;
...
/************************ for FM on off  tune rssi***********************/
    else if (strncmp(*argv, "fm_on", 5) == 0)
    {
        BTENG_LOGD("recv fm_on.");

        bt_mode_testing = FM_MODE_FM_ON;
        err = bteng_fm_on();
    }
    else if (strncmp(*argv, "fm_off", 6) == 0)
    {
        BTENG_LOGD("recv fm_off.");

        bt_mode_testing = FM_MODE_FM_OFF;
        err = bteng_fm_off();
    }
   else if (strncmp(*argv, "tune", 4) == 0 && argc > 1)
	{
		BTENG_LOGD("recv fm_tune freq.\n");

		freq = atoi(argv[1]);
		BTENG_LOGD("recv fm_tune freq=%d\n",freq);
		bt_mode_testing = FM_MODE_FM_TUNE_FREQ;
		bteng_fm_tune(freq);	

	}
   else if (strncmp(*argv, "getrssi", 7) == 0)
   	{
        BTENG_LOGD("recv fm_get_rssi.");

        bt_mode_testing = FM_MODE_FM_GETRSSI;
        err = bteng_fm_get_rssi();
   	}
