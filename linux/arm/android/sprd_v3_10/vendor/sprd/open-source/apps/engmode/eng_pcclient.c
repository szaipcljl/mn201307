...
static int eng_parse_cmdline(struct eng_param * cmdvalue)
{
    ...
    if (fd >= 0) {
		...
		turnoff_calibration_backlight();
                ...
                str = strstr(cmdline, "eng_device");

		/*device=device. Example: eng_device=146*/
		if(str != NULL) {
...
int main (int argc, char** argv)
{
	...
	//add the UART port both in cali mode and normal mode
	if(CONNECT_UART == cmdparam.connect_type){
            ENG_LOG("CONNECT_UART = %d\n",cmdparam.connect_type);
