...
#if  1//def CONFIG_TOUCHSCREEN_FIRMWARE_UPGRADE //huafeizhou151202 mod
static unsigned char CTPM_FW[] = {
//#include "FT6336S_OLD_802_XFY_B5020_ID0X31_Ver0X15_20150902_app.i"
};
#else
static unsigned char CTPM_FW[] = {
};
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	fts_read_reg(client, FT5x0x_REG_FW_VER, &uc_tp_fm_ver);
			  ...
			  u32 dw_lenth)
{
	...
	if(pbt_buf[0] != 0x02)
	{
		pr_info("[FTS] FW first byte is not 0x02. so it is invalid \n");
		return -1;
	}
	...
	if(dw_lenth > 0x11f)
	{
		...
		{
			pr_info("[FTS] Fw length is invalid \n");
			return -1;
		}
	}
	else
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			...
			pr_info("[FTS] Step 3: GET CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		} else {
	...
	for(i = 0;i < 200;i++)
	{
		...
		{
			pr_info("[FTS] erase app finished \n");
			break;
		}
		msleep(50);
	}
	...
	for (j = 0; j < packet_number; j++) {
		...
		for(i = 0;i < 30;i++)
		{
			...
			{
				pr_info("[FTS] write a block data finished \n");
				break;
			}
			msleep(1);
		}
...
int fts_6x06_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			&& reg_val[1] == fts_updateinfo_curr.upgrade_id_2) {
			pr_info("[FTS] Step 3: CTPM ID OK ,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		} else {
...
int  fts_5x46_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	if(i_ret == 0)
	{
		pr_info("[FTS] hid change to i2c fail ! \n");
	}
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if(i_ret < 0)
		{
			pr_info("[FTS] failed writing  0x55 and 0xaa ! \n");
			continue;
		}
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			&& reg_val[1] == fts_updateinfo_curr.upgrade_id_2) {
			pr_info("[FTS] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		} else {
					...
					char *buf)
{
	...
	if (fts_read_reg(client, FT5x0x_REG_FW_VER, &fwver) < 0)
