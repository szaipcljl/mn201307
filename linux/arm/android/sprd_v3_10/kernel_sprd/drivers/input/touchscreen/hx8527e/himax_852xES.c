...
u8 himax_sensor_id=0;//huafeizhou150624 add
u8 himax_cfg_ver=0x00;//huafeizhou150624 add

static ssize_t tp_information_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "CTP: himax 852Xes ID:0x%x\nFw_ver:0x%02x%02x,Cfg_ver:0x%02x\n", private_ts->vendor_sensor_id,private_ts->vendor_fw_ver_H,private_ts->vendor_fw_ver_L,private_ts->vendor_config_ver);
}
...
static void himax_info_init(void)
{
    int ret;
    ...
    {
        I("failed to create board_properties\n");
    }
}
#endif
...
static int himax_input_register(struct himax_ts_data *ts)
{
	...
	if (ts->protocol_type == PROTOCOL_TYPE_A) {
		...
		input_mt_init_slots(ts->input_dev, ts->nFinger_support,flags);//add flags,150821 
	}
...
HW_RESET_ACTIVATE = 0;
	...
	himax_info_init();	//huafeizhou150831 add
