...
static struct spi_board_info gf318m_spi[] __initdata = {
        [0] ={
            .modalias  = "gf318m", 
...
#define GF_FF_KEY           KEY_POWER //KEY_F10
...
#if 0
#define GPIO_INDEX_INT_EIC		(0)
#define GPIO_INDEX_3V3LDO		(1)
#define GPIO_INDEX_RST			(2)
#define GPIO_INDEX_INT_GPIO	(3)
#define GPIO_INDEX_VERSION		(4)

int gf_version_pin = -1;
int gf_miso_pin = -1;
int gf_3v3ldo_pin = -1;
#endif
...
#if 0
int sprd_goodix_parse_dts_gpio(void)
{
	...
	/*parse 3v3 ldo pin */
	gf_3v3ldo_pin = of_get_gpio_flags(np, GPIO_INDEX_3V3LDO, &flags);
	if (gf_3v3ldo_pin < 0) {
		printk("sprd:%s:Failed to get version gpio from dts: %d\n",__func__, gf_version_pin);
		return gf_3v3ldo_pin;
	}
	printk("sprd:%s:3v3 ldo gpio = %d.\n",__func__,gf_3v3ldo_pin);

	error = gpio_request(gf_3v3ldo_pin,"gpio_goodix_3v3ldo");
	if (error < 0){
		printk("sprd:%s:gpio_request failed!\n",__func__);
	//	return error;
	}
	error = gpio_direction_output(gf_3v3ldo_pin, 1);
	if(error){
		printk("sprd:%s,gpio_direction_output failed.\n",__func__);
	}
	
	/* parse version pin */
	gf_version_pin = of_get_gpio_flags(np, GPIO_INDEX_VERSION, &flags);
	if (gf_version_pin < 0) {
		printk("sprd:%s:Failed to get version gpio from dts: %d\n",__func__, gf_version_pin);
		return gf_version_pin;
	}
	printk("sprd:%s:version gpio = %d.\n",__func__,gf_version_pin);
	
	error = gpio_request(gf_version_pin,"gpio_goodix_int_version");
	if (error < 0){
		printk("sprd:%s:gpio_request failed!\n",__func__);
	//	return error;
	}
	
	gpio_direction_input(gf_version_pin);
	value = gpio_get_value(gf_version_pin);
	
	/* parse irq pin */
	if(value == 1)
		gf_irq_pin = of_get_gpio_flags(np, GPIO_INDEX_INT_EIC, &flags);	//eic async pin
	else if (value == 0)
		gf_irq_pin = of_get_gpio_flags(np, GPIO_INDEX_INT_GPIO, &flags);	//gpio 85
	...
	printk("sprd:%s:irq gpio = %d.\n",__func__,gf_irq_pin);

	error = gpio_request(gf_irq_pin,"gpio_goodix_int");
	if (error < 0){
		printk("sprd:%s:gpio_request failed!\n",__func__);
	//	return error;
	}

	/*transfer to irq number */
	gpio_direction_input(gf_irq_pin);
	gf_irq_num = gpio_to_irq(gf_irq_pin);
	if(gf_irq_num < 0){
		printk("sprd:%s:gpio_to_irq failed: %d.\n",__func__,gf_irq_num);
		return gf_irq_num;
	}	
	printk("sprd:%s:irq number = %d.\n",__func__,gf_irq_num);
	
	/* parse reset pin */
	gf_rst_pin = of_get_gpio_flags(np, GPIO_INDEX_RST, &flags);
	if(gf_rst_pin < 0){
		printk("sprd:%s,Failed to get reset gpio from dts: %d.\n",__func__,gf_rst_pin);
		return gf_rst_pin;
	}
	printk("sprd:%s:reset gpio = %d.\n",__func__,gf_rst_pin);
	
	error = gpio_request(gf_rst_pin,"gpio_goodix_reset");
	if (error < 0){
		printk("sprd:%s:gpio_request failed!\n",__func__);
	}
	
	error = gpio_direction_output(gf_rst_pin, 1);
	if(error){
		printk("sprd:%s,gpio_direction_output failed.\n",__func__);
	}
	...
	u16 addr, u32 data_len, u8 *rx_buf)
{
    ...
    ret = spi_sync(gf_dev->spi, &msg);
    if(ret == 0){
	//ret = msg.actual_length - 1;//GF_RDATA_OFFSET;   data_len的长度为1
	ret = data_len;
    }
#endif
    gf_debug(SPI_DEBUG, "ret = %d, actual_length = %d\n", ret, msg.actual_length);
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_CMD:
		...
		gf_debug(DEFAULT_DEBUG,"gf_ioctl Write data from 0x%x, len = 0x%x\n", ioc->addr, ioc->len);
		if(ioc->addr == 0x8043)
		{
			temp_buf=(void __user*)(unsigned long)ioc->buf;
			gf_debug(DEFAULT_DEBUG," temp_buf=%x,%x,%x,%x\n",temp_buf[0],temp_buf[1],temp_buf[2],temp_buf[3]);
		    gf_dev->mode = temp_buf[0];
		    gf_debug(DEFAULT_DEBUG,"set mode 0x%x \n", gf_dev->mode);
		}
...
void sprd_goodix_irq_bottom_func(struct work_struct * work)
{
	
    struct gf_dev *gf_dev = NULL;
    gf_dev = container_of(work, struct gf_dev, gf318m_irq_work);   
    u8 mode = 0x80;
    u8	status;
	...
	//pr_info("Invalid IRQ = 0x%x\n", status);
	enable_irq(gf_irq_num);
	return ;
    } 
    gf_spi_read_byte(gf_dev, GF_MODE_STATUS, &mode);
    gf_debug(SUSPEND_DEBUG, "status = 0x%x, mode = %d\n", status, mode);
    printk("status = 0x%x, mode = %d\n", status, mode);
...
static irqreturn_t gf_irq(int irq, void* handle)
{
	...
	if(gf318m_wakeup){																						 //add by xym
		disable_irq_nosync(gf_irq_num);
		schedule_work(&gf_dev->gf318m_irq_work);
	}
...
static int  gf_probe(struct spi_device *spi)
{
	...
	sprd_goodix_parse_dts_gpio();
	
    /* Allocate driver data */
    gf_dev = kzalloc(sizeof(*gf_dev), GFP_KERNEL);
    if (!gf_dev){
	pr_warn("Failed to alloc memory for gf device.\n");
	FUNC_EXIT();
	return -ENOMEM;
    }
	...
	gf_dev->input->name = "sprd-gf318m-key";
...
//	gf_irq_cfg();
	...
	INIT_WORK(&gf_dev->gf318m_irq_work, sprd_goodix_irq_bottom_func);
...
static int __init gf_init(void)
{
     ...
     */
    spi_register_board_info(gf318m_spi, ARRAY_SIZE(gf318m_spi));
