...
#define REG_ADDR_BIT8_WR 'o'
#define REG_ADDR_BIT8_RD 'O'
#define REG_ADDR_BIT16_WR 'x'
#define REG_ADDR_BIT16_RD 'X'
...
int program_register_for_913_ar0144()
{
	/*
...
int main(int argc, char *argv[])
{
	...
	if (argc < 5 || (*argv[1] != REG_ADDR_BIT8_WR && *argv[1] != REG_ADDR_BIT16_WR \
				&& *argv[1] != REG_ADDR_BIT8_RD && *argv[1] != REG_ADDR_BIT16_RD )) {
		printf("please pass the param:\n");
		printf("./i2c-test o i2c_addr 8bit_register_addr val\n");
		printf("./i2c-test x i2c_addr 16bit_register_addr val\n");
		printf("./i2c-test O i2c_addr 8bit_register_addr 0\n");
		printf("./i2c-test X i2c_addr 16bit_register_addr 0\n");
		printf("write 914 reg[0x61]: ./i2c-test x 0x61 0x8 0x36\n");
		printf("write 913 reg[0x5d]: ./i2c-test x 0x5d 0x8 0x36\n");
		printf("write ar0144 reg[0x3270]: ./i2c-test x 0x10 0x3270 0x8\n");

		exit(1);
	}

	program_register_for_913_ar0144();
	//sleep(1);

	sscanf(argv[2],"%x", &i2c_addr); //reg
	sscanf(argv[3],"%x", &reg_addr); //reg
	sscanf(argv[4],"%x", &val); //write val
