...
static int mmc_reset_for_rpmb(struct mmc_host *host, u8 part_config)
{
	int err = 0;
	...
	if (((part_config & 0x07) == EXT_CSD_PART_CONFIG_ACC_RPMB) &&
		...
		host->card->flags |= MMC_DISABLE_HS200_FOR_RPMB;
				      ...
				      struct mmc_blk_data *md)
{
	...
	if (mmc_card_mmc(card)) {
		...
		if (mmc_reset_for_rpmb(card->host, part_config))
...
#define RPMB_WRITE_TEST
#ifdef RPMB_WRITE_TEST
int write_cnt = 0;
#endif
