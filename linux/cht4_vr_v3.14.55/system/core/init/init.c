...
void notify_service_state(const struct service *svc, const char *state)
{
    ...
    if ((len + 10) > PROP_NAME_MAX)
        return;
    snprintf(pname, sizeof(pname), "init.svc.%s", svc->name);
...
void service_start(struct service *svc, const char *dynamic_args)
{
    ...
    if (properties_inited())
        notify_service_state(svc, "running");
}
...
static void service_stop_or_reset(struct service *svc, int how)
{
    ...
    if (svc->pid) {
        ...
        notify_service_state(svc, "stopping");
    } else {
        notify_service_state(svc, "stopped");
    }
}
...
#if defined(HAL_AUTODETECT) && !defined(HAL_AUTODETECT_PROPERTIES_DISABLED)
extern void autodetect_properties(bool get_dpi);
#endif
