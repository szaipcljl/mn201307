...
static sensor_state_t* get_sensor_state_with_name(char *name)
{
        ...
        for (i = 0; i < current_sensor_index; i ++) {
				...
				ALOGE("sensor_name %s, sensor_list_name=%s\n", name, sensor_list[i].name );
                if (strcmp(sensor_list[i].name, name) == 0) {
...
static void reset_sensorhub()
{
	...
	//spi:vr-sensor-spi-ipc	
		if( strstr(magic_string, "vr") != NULL ){
			found = 1;
...
static int fake_sensor_enumeration()
{
	int current_sensor_index = 0;
 	int i;
   	...
   	struct sensor_info fake_sensor_table[] = {
	    ...
	    [0]={
		    .id=1,
		    .freq_max=400,	// -1, means no fixed data rate
		    .name = "ACCEL",
	    },
	    [1]={
		    .id=2,
	    ...
	    [2]={
		    .id=3,
		    .freq_max=100,	// -1, means no fixed data rate
		    .name = "COMPS",
	    },
		[3]={
			.id = 4,
	...
	for(i=0; i<sensor_num; i++)
	{
		...
		strcpy(sensor_list[current_sensor_index].name, fake_sensor_table[i].name);
		
		ALOGI("sensor_name: %s, sensor id is %d, freq_max is %d, current_sensor_index is %d\n",fake_sensor_table[i].name, 
																							   fake_sensor_table[i].id, 
																							   fake_sensor_table[i].freq_max,
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		//TODO: skip CMD_DDR and CMD_RESET before FW ready
		//setup_psh();
		...
		if(platform == CHERRYTRAIL){
			log_message(DEBUG,"platform is cherrytrail\n");
		}
