...
static char g_stream_sensor_array[256];

/*
static int resp_id_to_cmd_id(int resp_id)
{

}

static int check_if_received_ack(int cmd_id, int sensor_id)
{
			...
			struct ia_cmd *cmd, int len)
{
    ...
    if (CMD_CFG_STREAM == cmd->cmd_id)
    {
        if (cmd->sensor_id < 256)
        {
            ...
            psh_err("set sensor_id %d stream ack state = FALSE\n",
                    cmd->sensor_id);
        }        
        else
        {
            psh_err("receive wrong sensor_id %d in CMD_CFG_STREAM\n",
                    cmd->sensor_id);
        }
    }
			...
			const char *str, size_t count)
{
    ...
    sscanf(str, "%s %d %d", dbg_cmd, &dbg_param1, &dbg_param2);
    dev_err(dev, "recv dbg command: %s param1 = %d param2 = %d\n",
            dbg_cmd, dbg_param1, dbg_param2);
    
    if (0 == strcmp(dbg_cmd, DBG_CMD_STREAM_START))
    {
        ...
        //for reuse exist scheme
        psh_ia_data->is_in_debug = 0;

    }
    else if (0 == strcmp(dbg_cmd, DBG_CMD_STREAM_STOP))
    {
        memset(&cmd, 0, sizeof(cmd));
    ...
    else if (0 == strcmp(dbg_cmd, DBG_CMD_XFER))
    {
        int loop_i = 0;

        psh_ia_data->test_cmd.tran_id = 1;
        psh_ia_data->test_cmd.cmd_id = 2;
        psh_ia_data->test_cmd.sensor_id = 3;
        
        for (loop_i = 0; loop_i < CMD_PARAM_MAX_SIZE; ++loop_i)
        {
            psh_ia_data->test_cmd.param[loop_i] = 20 + loop_i;
        }

        psh_ia_data->debug_send_count = dbg_param1;

        psh_ia_data->debug_scheme = DBG_CMD_XFER;
    ...
    {
        psh_ia_data->debug_scheme = DBG_CMD_STRESS;
        if (0 == dbg_param1 && 0 == dbg_param2)
        {
            psh_ia_data->stress_test_step = STRESS_STEP_QUIT;            
        }
        else
        {
            psh_ia_data->stress_test_step = STRESS_STEP_0_PREAPRE_DATA;

            dbg_param1 = MAX(1, dbg_param1);
    ...
    else if (0 == strcmp(dbg_cmd, DBG_CMD_STREAM_FREQ))
    {
        ...
        poller_set_frequency(&lp_psh_info->poller_worker,
                             dbg_param1);
...
int ia_handle_frame(struct psh_ia_priv *psh_ia_data, void *dbuf, int size)
{
	...
	case RESP_CMD_ACK:
        ...
        if (CMD_CFG_STREAM == cmd_ack->cmd_id)
        {
            if (resp->sensor_id < 256)
            {
                ...
                psh_err("set sensor_id %d stream state = TRUE\n", resp->sensor_id);
            }
            else
            {
                psh_err("received cmd_ack with wrong sensor id: %d\n", resp->sensor_id);
            }
        }
    ...
    case RESP_STREAMING:
        ...
        {
            /*
            if (!g_stream_sensor_array[resp->sensor_id])
            {
                psh_err("drop stream for sensor_id %d because not receive ack. \n",
                        resp->sensor_id);
                return 0;
                }
            */
...
static void destroy_sysfs_interface(struct device* dev)
{
...
int psh_ia_common_init(struct device *dev, struct psh_ia_priv **data)
{
	...
	*data = psh_ia_data;
    ...
    ret = create_sysfs_interface(dev);
